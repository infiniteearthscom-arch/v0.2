...
import { galaxyAPI, harvesterAPI, contractsAPI, basesAPI } from '@/utils/api';
      ...
      {systems.map(sys => {
        ...
        return (
          ...
          >
            ...
            {/* Base marker -- house at the lower-left when I own a base here. */}
            {myBases[sys.id]?.length > 0 && (
    ...
    <ModalOverlay windowId="galaxyMap" title="Galaxy Map" icon="🌌" accent="#8844ff">
      <div className="flex h-full gap-0">
        ...
        <div className="flex-1 relative overflow-hidden">
          ...
          {/* Zoom display */}
          <div className="absolute bottom-2 left-2 text-[0.8rem] text-slate-600 font-mono">
        ...
        <div className="w-[200px] border-l border-slate-700/50 bg-slate-900/50 p-3 overflow-y-auto flex-shrink-0">
          {selectedSys ? (() => {
            ...
            <div className="space-y-2">
              ...
              {!isDiscovered && (
                <div className="text-[0.8rem] text-slate-500 italic">
              ...
              <div className="space-y-1 text-[0.8rem]">
                ...
                {myBases[selectedSys.id]?.length > 0 && (
                  <div className="flex justify-between">
              ...
              {isDiscovered && selectedSys.resourceProfile && (
                <div className="space-y-1 pt-1 border-t border-slate-700/30">
                  <div className="text-[0.8rem] text-slate-600 uppercase tracking-wider">Resources</div>
                  {Object.entries(selectedSys.resourceProfile).map(([key, val]) => (
                    <div key={key} className="flex items-center gap-2">
                      <span className="text-[0.8rem] text-slate-500 w-12 capitalize">{key}</span>
              ...
              {isDiscovered && selectedSys.hasJumpGate && selectedSys.jumpConnections.length > 0 && (
                <div className="space-y-1 pt-1 border-t border-slate-700/30">
                  <div className="text-[0.8rem] text-slate-600 uppercase tracking-wider">Jump Connections</div>
                  {selectedSys.jumpConnections.map(connId => {
                    ...
                    return (
                      <div key={connId}
                        className="text-[0.8rem] text-slate-400 hover:text-cyan-300 cursor-pointer"
            ...
            <div className="text-center py-8">
              ...
              <div className="mt-6 space-y-1.5 text-left">
                <div className="text-[0.8rem] text-slate-600 uppercase tracking-wider mb-2">Factions</div>
                ...
                {Object.entries(STAR_COLORS).map(([type, color]) => (
                  <div key={type} className="flex items-center gap-2">
                    ...
                    <span className="text-[0.8rem] text-slate-400">{type.replace('_', ' ')}</span>
                  </div>
                ))}
              </div>
            </div>
