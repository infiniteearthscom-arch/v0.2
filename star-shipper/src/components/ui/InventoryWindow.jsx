...
export const InventoryWindow = () => {
  ...
  // Tooltip show/hide goes through the singleton CargoTooltipLayer —
  // plain function calls, no state on this component, so hovering
  // across tiles doesn't re-render the whole grid (the old lag bug).
  ...
  // A tooltip must not outlive the window (mouseleave doesn't fire on
  // unmounted tiles — e.g. closing the panel while hovering).
  ...
  return (
    <ContextPanel windowId="inventory" title="Cargo" icon="📦" accent="#f59e0b" width={420}>
      <div className="flex flex-col h-full relative" ref={containerRef}>
        ...
        {/* Hover tooltip now renders via the singleton CargoTooltipLayer
