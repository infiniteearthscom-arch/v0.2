...
const ShipSelector = ({ ships, selectedId, onSelect, hulls, onBuyHull }) => {
  ...
  return (
    <div className="space-y-2">
      <div className="text-[0.9rem] text-slate-500 uppercase tracking-wider">Your Fleet</div>
      {ships.map(ship => (
        ...
        >
          ...
          <div className="text-[0.9rem] text-slate-500">{ship.hull_name || 'Unknown'} • {ship.hull_class || ''}</div>
        </button>
      ))}
      {ships.length === 0 && (
        ...
        <div className="space-y-1 mt-1">
          {hulls.map(h => (
            ...
            >
              ...
              <div className="text-[0.9rem] text-slate-500">{h.class} • {(h.slots || []).length} slots</div>
            </button>
          ))}
        </div>
      )}
...
const SlotInfo = ({ slot, module }) => {
  if (!slot) return <div className="text-center text-slate-600 text-[0.9rem] py-3">Hover a slot for details</div>;
  ...
  return (
    <div className="p-2 rounded border text-xs" style={{ borderColor: st.color + '44', background: st.color + '08' }}>
      <div className="flex items-center gap-2 mb-1">
        ...
        {slot.required && <span className="text-[0.9rem] text-red-400 font-bold">REQUIRED</span>}
      </div>
      <div className="text-[0.9rem] text-slate-500">Size: {slot.w}×{slot.h}</div>
      {module ? (
        <div className="mt-1.5">
          ...
          {/* Description -- pulled from module_types via the JOINed
              ...
              tooltips show. */}
          {module.description && (
            <div className="text-[0.9rem] text-slate-400 mt-1 leading-snug italic">
              {module.description}
            </div>
          )}
          {avgQ !== null && (
            <div className="flex items-center gap-2 mt-1.5">
              <span className="text-[0.9rem] text-slate-500">Quality:</span>
              <div className="flex-1 h-1.5 bg-slate-800 rounded-full overflow-hidden">
                <div className="h-full rounded-full" style={{ width: `${avgQ}%`, backgroundColor: qColor }} />
              </div>
              <span className="text-[0.9rem] font-medium" style={{ color: qColor }}>Q{avgQ}</span>
            </div>
          )}
          ...
          {module.base_stats && (
            <div className="mt-1.5 pt-1.5 border-t border-slate-700/30">
              {Object.entries(module.base_stats).map(([key, val]) => {
                ...
                return (
                  <div key={key} className="flex justify-between text-[0.9rem] py-px">
                    <span className="text-slate-500">{label}</span>
                    <span>
                      <span className="text-slate-200 font-medium">{fmtStatValue(scaled, meta)}</span>
                      {differs && (
                        <>
                          <span className="text-slate-600 ml-1.5">(base {fmtStatValue(val, meta)})</span>
          ...
          {slot.type === 'weapon' && (() => {
            ...
            return (
              <div className="mt-1.5 pt-1.5 border-t border-slate-700/30">
                <div className="text-[0.9rem] text-slate-500 mb-0.5 uppercase tracking-wide">
                  ...
                  {eff.rows.map(r => {
                    ...
                    return (
                      <div key={r.layer} className="flex-1 text-center rounded px-1 py-0.5" style={{ background: c + '14' }}>
                        <div className="text-[0.9rem] text-slate-400">{r.label}</div>
                        <div className="text-[0.9rem] font-bold" style={{ color: c }}>×{r.mult.toFixed(2)}</div>
                      </div>
                    );
                  })}
                </div>
          ...
          <div className="text-[0.9rem] text-slate-600 mt-1">Click to unfit</div>
        </div>
      ) : (
...
const ModuleShop = ({ moduleTypes, onBuy }) => {
  ...
  return (
    <div className="space-y-2">
      <div className="text-[0.9rem] text-slate-500 uppercase tracking-wider">Station Modules</div>
      {Object.entries(grouped).map(([type, mods]) => {
        const st = SLOT_TYPES[type] || { color: '#888', name: type };
        return (
          <div key={type}>
            <div className="text-[0.9rem] font-medium mb-1" style={{ color: st.color }}>{st.name}</div>
            {mods.map(m => (
              ...
              >
                ...
                <div className="text-[0.9rem] text-slate-500">T{m.tier} • {m.description}</div>
              </button>
            ))}
          </div>
        );
    ...
    <div className="flex flex-col h-full min-h-0" style={{ background: '#0c1018', borderRadius: 6, border: '1px solid #1e293b' }}>
      ...
      <div className="flex items-center justify-between px-2.5 py-1.5 border-b border-slate-700/40">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[0.9rem] font-bold uppercase tracking-wider text-slate-300">Fittable Modules</span>
        </div>
        <button
          onClick={onRefresh}
          ...
          return (
            <div key={slotKey} className="mb-2">
              ...
              <div className="flex items-center gap-1.5 px-1 py-0.5 mb-1">
                ...
                <span className="text-[0.9rem] font-bold uppercase tracking-wider" style={{ color: st.color }}>{st.name}</span>
                <span className="text-[0.9rem] text-slate-600">({items.length})</span>
              </div>
      ...
      <div className="h-full flex flex-col gap-2 text-cyan-100">
        <div className="flex-1 flex gap-3 min-h-0">
          ...
          <div className="w-44 flex-shrink-0 flex flex-col gap-2 overflow-y-auto">
            ...
            {/* Slot legend */}
            <div className="mt-2">
              <div className="text-[0.9rem] text-slate-500 uppercase tracking-wider mb-1">Slot Types</div>
              {Object.entries(SLOT_TYPES).map(([key, st]) => (
                <div key={key} className="flex items-center gap-1.5 mb-0.5">
                  ...
                  <span className="text-[0.9rem] text-slate-400">{st.name}</span>
                </div>
              ))}
            </div>
          </div>
