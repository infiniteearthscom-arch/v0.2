...
// A single ship's contribution to the fleet's pooled hull (combat F3
// player attrition). MUST stay the same math `computeFleetStats` sums
// into totalHull, so the per-ship death thresholds partition the pool
// exactly.
export const getShipHullContribution = (ship) => {
  const base = getShipBaseStats(ship);
  const mods = getShipModuleBonuses(ship);
  return base.hull + mods.hull;
};
