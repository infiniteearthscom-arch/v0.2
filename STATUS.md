...
- **NEXT (the new combat core) — fleet-vs-fleet collective entities.** Both player AND enemy fleets become single entities with pooled Shield→Armor→Hull and **visual attrition** (ships peel off/explode as the pool degrades — escorts first, flagship last; DPS decays). Per-fleet 3-bar readout + per-fleet targeting. The damage triangle becomes a readable shifting sequence within a fight. Player fleet attrition → pod on last ship. Biggest combat lift (touches the protected loop + rendering); spec the hull-pool→ship-death threshold mapping first. See spec §A2.
