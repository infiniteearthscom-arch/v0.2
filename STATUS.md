...
- **IN BUILD (the new combat core) — fleet-vs-fleet collective entities.** Full mechanic specced in `docs/combat-attrition-spec.md`. Build order F1→F4.
  ...
  - **NEXT — F3 player attrition + pod** (player wingmen peel off on thresholds → pod on flagship; needs new `POST /fleet/lose-ship` endpoint to destroy a non-active ship + eject its wreck), then **F4 loot manifest/targeting polish**.
