...
**Last updated:** 2026-06-10 (Combat **F3 player attrition + pod** built + build-verified — see In progress. Same session: UI sizing pass — context panels fit content height, Leaderboards/Mail/Bounties/Corp converted from centered modals to left-anchored context panels, cargo grid 6→8 cols.)
...
- DB schema: applied through migration **061**; **062 (five-tier modules) is authored and pending `npm run db:migrate` after its deploy** (009 was skipped)
...
- **IN BUILD (the new combat core) — fleet-vs-fleet collective entities.** Full mechanic specced in `docs/combat-attrition-spec.md`. Build order F1→F4.
  ...
  - **NEXT — F4 polish** (loot manifest + flagship payout ties to A3 stakes; targeting refinements; battlefield tint). Playtest note for F3: enemy damage tuning now matters more — a losing fight costs multiple real ships.
