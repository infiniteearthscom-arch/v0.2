...
**Combat Depth & Galaxy Progression overhaul — IN BUILD.** Full consolidated spec: **`docs/combat-progression-spec.md`**. **Combat P0+P1 (armor layer + damage triangle) is implemented + build-verified locally, pending push + live playtest** (2026-06-02). New `utils/combat.js` holds the shared `applyDamage()` + `DAMAGE_MATRIX` (laser→armor, kinetic→shield, missile→hull); defense now resolves shield→armor→hull top-down. Enemies gained an armor layer (redistributed from hull so neutral EHP is unchanged; heavies armor-tanky, lights bare) + a `weaponType`; player armor comes from `fleetStats.totalArmor` (armor modules finally do something). HUD shows an armor bar when fitted; enemy health bars show an armor segment; Ship Builder weapon tooltip shows the per-layer effectiveness triangle. **No migration** (constant matrix). **Combat P2a (Starfield power pips) also implemented + build-verified 2026-06-02, pending push + playtest** — fleet-wide pip pool (LAS/BAL/MIS/SHD/ENG) from reactor count; soft-floor model (starved weapons fire slowly, never offline); even split = neutral (ignoring the panel changes nothing). New `utils/powerPips.js` + bottom-left `PowerPanel` (+/− and keys 1–5); weapon fire rate, shield regen+resist, and engine speed all scale with allocation. No migration (pool client-derived for now; P2b makes it server-authoritative via migration 062 + reactor quality). Next: P2b, then P2c (TTK retune), and the zoning thread per spec §7. This is the agreed next major direction (supersedes the "playtest social vs pivot" open question below). It couples two threads into one progression engine: (A) combat depth — armor layer + damage-type×defense-layer triangle, Starfield-style pip power management, full stakes (cargo/module ejection + server-validated loot); (B) galaxy zoning — 5 chokepoint-gated difficulty tiers, hard-gated rare resources, warp-drive-range travel, four enemy-scaling mechanics — feeding (C) the crafting loop. First buildable slice = combat **P0+P1** (armor layer + damage triangle; no migration, instantly playtestable). See the spec's §7 for the full build sequence. Design decisions also captured in memory (`project_combat_overhaul.md`, `project_galaxy_zoning.md`).
