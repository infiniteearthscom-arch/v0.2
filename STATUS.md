...
**Last updated:** 2026-05-27 (Cargo skills branch shipped; Realtime Presence Phase 1 spec locked for tonight's session)
...
- **Realtime multiplayer Phase 1: Presence** — locked spec below. Picking this up next session (2026-05-27 evening per user).
...
### Realtime multiplayer (Phase 1: Presence)

Specced in chat 2026-05-27. First slice of the multi-phase road to shared combat: two players in the same system see each other's ships moving in real time. **No shared enemies, no combat sharing, no PvP** -- that's Phase 2/3. Pure presence so the transport + room model gets validated before stakes are high.

**Goal:** see peer ships in SystemView, with name tag + accurate position/rotation/velocity, rendered via the existing `shipRenderer.js` (so peer fits look like proper ships).

**Out of scope (Phase 1):** galaxy-map presence, peer combat damage, peer chat, peer wingmen, server-authoritative position validation.

**Locked architecture decisions:**

| Decision | Pick | Why |
|---|---|---|
| Transport | `socket.io` server + `socket.io-client` | Rooms + auto-reconnect + handshake auth come free. |
| Co-host | Same Node process, same port (`io.attach(httpServer)`) | DO routes one port per service. |
| Authority | Client-authoritative position in Phase 1 | Pure relay; cheaters can teleport a visual ghost with no gameplay impact. Phase 3 validates. |
| Snapshot cadence | 5 Hz (200ms) | Tiny payloads; client interpolates to 60fps via `gameTimeRef`. |
| Server tick | None in Phase 1 (pure relay) | Phase 2 introduces a tick for server-owned entities. |
| Room key | `system:${procedural_id}` (`system:sol` for Sol) | Stable, derivable, no extra table. |
| Per-user cap | One active socket per user (kick old on new connect) | Two tabs both broadcasting becomes a "where am I really" puzzle. |
| Stale eviction | 500ms no update -> fade; 5s -> remove | Matches packet loss reality. |

**Message protocol:**

```
Client -> Server:
  'enter_system'  { system_id }
  'leave_system'  { system_id }
  'pos'           { x, y, vx, vy, rot, ship_visual_v }

Server -> Client:
  'peer_join'   { user_id, name, ship_visual }
  'peer_leave'  { user_id }
  'peers'       { peers: [{ user_id, x, y, vx, vy, rot, ts }, ...] }
```

`ship_visual_v` is a monotonic counter the client bumps on re-fit / ship change so peers know to refetch the descriptor without it riding every tick. `ship_visual = { hull_type_id, accent_color, ship_name, fitted_summary }`. `ts` is server-stamped for interpolation + stale detection. Server rate-limits `pos` to 10 Hz/socket as a defensive cap.

**File layout (server):**

```
star-shipper-server/src/realtime/
  index.js          # io = new Server(httpServer); mount + export
  auth.js           # io.use middleware: validate JWT from handshake.auth.token
  presence.js       # enter_system / leave_system / pos / disconnect handlers
  ship_visual.js    # query the peer's active ship (storage_body_id IS NULL) -> small descriptor
```

Existing `src/index.js` changes: `createServer(app)` instead of `app.listen`, then `realtime.attach(httpServer, app)`. **No DB writes in Phase 1** -- presence state lives in memory (`Map<userId, socketId>` + `Map<systemId, Set<userId>>`). Server restart = ~2s reconnect blip.

**File layout (client):**

```
star-shipper/src/
  utils/presence.js      # singleton socket wrapper: connect, enter/leave, sendPos, event subscribers
  hooks/usePresence.js   # subscribes to peer stream; returns peersRef (Map) + peerCount state
  components/system/SystemView.jsx  # changes:
                         #   - call usePresence(currentSystemId)
                         #   - in game loop: throttled sendPos at 5 Hz
                         #   - in render: iterate peersRef, lerp prev->next on gameTimeRef,
                         #     render via shipRenderer.js with peer chrome
```

`peersRef` must be a **ref, not state** (pitfall #6 -- physics/render reads from refs). React state is only for `peerCount` (HUD display) and connection status.

**Visual treatment for peer ships:**
- Same `shipRenderer.js` hull shape (peers see your fit, you see theirs -- feels real)
- Soft cyan glow halo (`#4488ff` at 40% alpha, 8px blur)
- Name tag above ship: small monospace, faded, ~25px offset
- Slightly desaturated fill (~85%) so peers don't fight your own ship's contrast
- **No** health bars, hit flashes, or combat indicators (Phase 3)
- Wingmen not rendered (active flagship only)
- Hover -> show name + stub `> PROFILE` link. Click does nothing in Phase 1.

**Edge cases handled:**

| Case | Behavior |
|---|---|
| Player A enters Sol, B already there | A gets `peer_join` for B (with ship_visual) on entry, then B's next `pos` broadcast lands. |
| Connection drops | After 5s no pos update, server emits `peer_leave`. Client fades the ghost over 500ms. |
| Player changes system | `leave_system` on old -> `enter_system` on new. Brief gap during galaxy flight is fine. |
| Player re-fits | Bump `ship_visual_v`, re-broadcast. Peers refetch the descriptor lazily. |
| Two tabs / accounts | Server kicks the older socket on new connect for the same userId. Old tab gets `'kicked'` event -> toast. |
| Server restart | Sockets auto-reconnect, re-emit `enter_system`. ~2s blip. |

**Open design questions to settle when work starts:**
1. Faction colors on peer ships (apply system faction tint, or always cyan)? Lean: cyan for Phase 1 -- pirate distinction matters more than faction nuance.
2. Sol clustering at high concurrency -- 20 players = 19 peers per Sol-player = ~4 KB/s per player. Fine, but cull farthest-30 if needed.
3. Galaxy fly-view presence -- defer (out of scope); flag for Phase 2.
4. DO websocket support -- current Basic plan supports WS. Confirm `VITE_API_URL` can build the WS URL (just swap `https://` -> `wss://`).

**Rollout plan:**
1. Ship server-only first (socket.io attached, no client integration). Manually `wscat` from local to verify auth + rooms work in prod.
2. Client behind a flag: `VITE_PRESENCE_ENABLED=true`. Push with flag off; flip on after manual verification.
3. Two-browser test (different browsers for cookie isolation, two accounts, both fly into Sol).
4. Kill-server reconnect test.
5. 4-player smoke test: 3 friends on simultaneously, Sol -> Alpha Centauri -> back. Watch for memory leaks in the server Maps.

**Effort estimate:** ~510 LOC, ~5 dev days.

| Piece | LOC | Time |
|---|---|---|
| Server: auth + presence | ~150 | 1 day |
| Client: socket wrapper + hook | ~180 | 1 day |
| SystemView render integration | ~120 | 1.5 days |
| Visual polish | ~60 | 0.5 day |
| Testing + deploy hardening | -- | 1 day |

**Phase 2/3 follow-ons (not specced yet, in priority order):**
- Phase 2: Shared enemies -- pirates owned by server, ~10Hz server tick, AI ports from `SystemView.jsx` to server. ~600 LOC client-side AI moves server-side. 2-3 weeks.
- Phase 3: Authoritative combat -- `POST /combat/fire` request/response, server validates range/cooldown/quality/LOS, broadcasts hits, kills credit firing fleet. Wreckage server endpoints (Known Issues) become required. PvP gate is an open call (opt-in flag vs high-sec/null-sec analog). 3-4 weeks.
- Phase 4: Polish -- client prediction + reconciliation, lag compensation, sweep telemetry for spectators, DO plan bump.
