...
**Pixel-art v2 tuning (owner playtest: "stations look flat", "animations stutter"). BUILT 2026-09-21, build-verified, NOT YET PUSHED. Client only.** (1) **Planet lighting decoupled from texture**: `getPlanetSheet(body, color)` now bakes an UNLIT 48-frame texture once per planet (materials at their bright step); `getShadeMask(size, hasRings, lightIdx)` is a one-frame disc of multiply factors (0.36/0.58/0.8/1.0 + checker dither) shared by every planet of that pixel size, composited over the texture with `mix-blend-mode: multiply` in SystemView. No re-bake when the star-facing light index changes (the old per-light-direction 24-frame sheets caused a hitch each time the terminator ticked). Spin raised to 0.05–0.2 rev/s → ~2–10 fps of rotation depending on size; frames 24→48 so each step is 7.5°. Known: emissive lava fissures darken on the night side under multiply (a glow pass could exempt them). (2) **Station shading**: upper-left light — edge pixels with open top/left get ×1.28, open bottom/right ×0.66, interior rows alternate ×1.0/×0.88 (panel seams), 1-px dark outline around the silhouette; windows/nav lights/accents stay emissive. (3) Frame counts doubled: stars 16 @ 10 fps, gate 12 @ 12 fps, warp 12 @ 14 fps.
