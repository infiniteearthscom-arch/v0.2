...
- DB schema: applied through migration **084** (085–089 authored 2026-09-25/26, pending deploy + migrate)  (deployed + migrated 2026-09-21; 009 was skipped). Always `npm run db:migrate` then `npm run db:verify` after a migration deploy — 064/065 and 068 each sat unapplied for weeks when that step was skipped.

---

## In progress

**Depot = cargo hold, drag and drop (owner 2026-09-26; migration 089). BUILT, build-verified, NOT YET PUSHED — run `npm run db:migrate` + `npm run db:verify`.** `player_base_inventory` gains `item_type` / `item_id` / `item_data` / `slot_index` (resource_type_id nullable; existing rows get positions). `lib/materials.js` owns the depot now (`depotStacks`, `depotUsed` — items 1 unit each, `depotAdd` / `depotAddItem`, `loadResourceStackAny` / `debitStackAny`). Base console depot panel: fleet hold and base hold as two `CargoGrid`s (new `components/items/CargoGrid.jsx`, InventoryWindow-style tiles + tooltip singleton); drag across = deposit / withdraw whole stack, drag within = move / swap / merge (`POST /bases/:id/depot/move`); the right column widens to 520 for it. Everything at the base uses the depot: foundry jobs + base upgrades already did; buildings fit from the depot (`/fit { depot_stack_id }`, "IN THE DEPOT" list on an empty plot); refinery quotes/queues depot stacks (`source`); the Crafting window lists depot resource stacks with a BASE badge while docked at your base and `/craft` consumes them (`ingredients[].source`). Foundry collect defaults to the depot for items too. Sealed contract freight cannot be deposited.

**The Foundry — base industry tree + base console (owner 2026-09-26; migration 088). BUILT, build-verified, NOT YET PUSHED — run `npm run db:migrate` + `npm run db:verify` after deploy.** Spec: **`docs/foundry-spec.md`**; authoring source `server/src/game/foundryTree.js` (088 generated from it). Five station families × five tiers: 23 stations (base modules, craft-only, `tech_foundry_1..5`), 36 processed materials (`resource_types` category `processed` + `tier`/`family`/`is_part` columns), 41 timed jobs (`foundry_recipes` → `player_foundry_jobs`, refining-v2 job model, inputs from depot then cargo, quality inherited, station Q sets speed), 9 craft-only PARTS that build stations and base tiers. **T2+ ship-module recipes rewritten** to processed materials (2 raw = 1) with `crafting_recipes.station_required` = the tier's bench; `/craft` refuses unless docked at an own base with that bench (Crafting window shows "⚙ Craft at your base"). **Base tiers 1–5** (4 plots per tier, Hub/Citadel via `tech_base_citadel`; costs in parts from depot+cargo via new `lib/materials.js`). **Base console** (`components/base/BaseWindow.jsx`, window `base`, opened from the planet Base tab): pixel portrait (`utils/pixelArt/baseArt.js`), plot grid by area, station panel (recipes / runs / queue / collect), depot, grade refinery, lab, new **Repair Shop** service (`/fitting/repair` accepts an own base with one, 25% off). `api/foundry.js` (`/catalog` public, `/status`, `/queue`, `/jobs/:id/cancel|collect`). Wreck drops: any non-part material can roll on a claim (`combat.js rollFoundryDrop`). Tooltips: Made at / Used for from the catalog (GameFrame `FoundryCatalogLoader` also registers the new resources for icons; new glyphs ingot / part / furnace / tank / vat / chip / bench / wrench). Reset wipes foundry jobs; `db:verify` checks 088. ⚠ Migration runs a PL/pgSQL rewrite over every T2+ ship-module recipe; existing pilots mid-craft on old raw recipes lose that path (materials stay in cargo). Not built: no-use consumables, T4/T5 blueprints, drone bay / vendor post, surface-vs-orbital bonuses.

**Hotbar (keys 1–5) + T targets the nearest hostile (owner 2026-09-26). BUILT, build-verified, NOT YET PUSHED. Client only.** New `components/hud/Hotbar.jsx`: five 38 px icon tiles bottom-right, immediately left of the System Map toggle (owner 2026-09-26: "same size, next to it"), number badge, cooldown overlay, locked tiles dimmed with a 🔒 and a "fit X" tooltip, drag to reorder, layout in localStorage (`hotbar.v1`, default Area Scan / Bulk Belt / Sweep / empty / empty; T is a hotkey only, not a tile; the Bulk Belt icon is ☄️ because 🪨 has no glyph on Windows 10). It REPLACES the bottom-right scan-ability tray; SystemView builds the ability list each render (area scan, bulk belt, system sweep, target-nearest) and exposes `activateSlot` / `targetNearest` through `hotbarActionsRef` so the keydown effect never holds a stale closure. Keys 1–5 activate, **T** designates the nearest living enemy in sensor range (uses `fleetSensorRange()`, so an active sweep sees everything) and pressing T again cycles to the next-nearest; both ignored while typing in an input. The owner's reason: clicking a moving sprite was too hard, so missiles (which need a held lock) were never fired. Consumables from the base industry tree will slot into the same bar. `HOTBAR_SIZE` is the one knob for growing it to 10.

**Cargo sort + filter (owner 2026-09-26). BUILT, build-verified, NOT YET PUSHED. Client only.** `InventoryWindow` gains a bar under the cargo meter: **filter chips** (All / Resources / Modules / Fuel / Probes / Harvesters / Freight — only kinds present, with counts; `cargoKind(stack)` classifies by item_type, item_category, slot_type and item_id), a **sort** select (Position = the player's own slot layout; Name A–Z; Type = kind → resource category + rarity / module slot + tier → name; Value = unit sell_price × quantity, richest first) and a **search** box. Position is the default; any other sort, a filter or a search shows an ARRANGED view (compact, padded to a full row) where drag-to-rearrange is refused with a hint but merges and trashing still work; the cross-window drag payload carries the REAL slot index. Filter + sort persist in localStorage (`cargo.view`); search resets per session. Also fixed the same day: the "mined out by another pilot" toast fired on your own rocks (the server broadcasts asteroid:update to the firing pilot too; the client now ignores its own events).

**Mining laser range is real (migration 087; owner 2026-09-25). BUILT, build-verified, NOT YET PUSHED — run `npm run db:migrate` + `npm run db:verify` after deploy.** Reach was a hardcoded 120 in SystemView; the lasers' `mine_range` stats (120 / 150 / 170) were ignored and no skill touched range. Now reach = **best fitted laser's `mine_range` × √quality × (1 + Beam Focusing %)**, fleet-wide best (pitfall #15), computed in new `utils/mining.js` (`getFleetMineRange`; older fits without a stats snapshot fall back to a per-id table) and read live for the click gate ("get within N units (M away)") and the per-frame release. **087** adds the Industry skill `ind_beam_focusing` (`mining_range_pct` +5 %/level; in `WIRED_BONUS_TYPES`). **Server rule:** the mine request now carries the fleet position; `/resources/asteroids/mine` computes the same range from `module_types.stats.mine_range` × √Q × the skill and rejects fires beyond range × 1.15 (`MINE_RANGE_SLACK`) with `out_of_range`; old clients that send no position skip the check until refreshed. `STAT_META` gains `mine_range` (√ scaling) so tooltips show the effective reach. Range is still measured from the flagship, not the ship carrying the laser (per-ship range = later polish). Next on this ladder: the **auto-miner** module (beams every rock in reach without click-targeting).

**Public bases + starbases (owner 2026-09-25). BUILT, build-verified, NOT YET PUSHED. Client + server, no migration.** Bases were private (every endpoint queried the caller's own rows). Now: `GET /bases/system/:id` (public projection via `publicBase`), `GET /bases/galaxy` (mine + others), `/here` gains `others`; the Base tab lists other pilots' bases on the planet; the galaxy map marks systems with others' bases (🏘 grey) + an "Other Bases" info row. **Orbital rule:** `planetHasStation(systemId, planetName)` (Sol → Earth; procedural → generator station bodies with `parentBody` = that planet) blocks orbital bases there (`orbital_blocked` reason in eligibility; 403 on build; the Base tab greys the button). **Starbases:** SystemView fetches `basesAPI.inSystem` on entry and appends every built orbital base as a station body (`id starbase_<baseId>`, `isStarbase`, orbits its planet at size+26, station sprite via `pickStationVariety`, owner tag under the label) — dockable by anyone. To keep the manifest / belt effects from re-running when starbases load, `currentSystem` is now `baseSystem` + starbases and those two effects read bodies through `currentSystemRef` and depend on the system id only. PlanetInteractionWindow: starbases never hit `/ensure-body`; docking shows a **Starbase** tab (owner portrait, tier, fitted modules, "docking only for now"). Not built: owner-run vendors / storage / services at starbases, surface-base markers in the system view, Pilots roster at starbases (needs a body row).

**Icon consistency sweep (owner 2026-09-25: "crafting icons match the item icons … consistency across the game"). BUILT, build-verified, NOT YET PUSHED. Client only.** Every item, module, resource and recipe glyph now comes from `utils/pixelArt/itemIcon.js` via `PixelItemIcon`: item + resource tooltips (previous push), Crafting window recipe rows / output header / selected-recipe header (`recipeIconSpec`, from output_item_id + item_data_defaults.slot_type + module_tier + module_stats.damage_type) and ingredient slots (`resourceIconSpecByName`), harvester cargo tiles, vendor Supplies rows, the Sell tab item rows, orbital-scan resource rows, deposit cards and the active-harvest header. No emoji or two-letter abbreviations remain on item surfaces; the emoji in `item_definitions.icon` is now unused by the client.

**Fix: asteroid tooltip stuck after the rock was mined out (owner report 2026-09-25). BUILT, NOT YET PUSHED. Client only.** A removed rock unmounts without firing onMouseLeave. SystemView now tracks the hovered rock id (`hoveredAsteroidRef`) and every removal path (own depletion response, the "depleted" error path, the shared `asteroid:update` event) calls `forgetAsteroid(id)`, which hides the tooltip if that rock was under the cursor.

**Refining v2 — base refineries, timed + queued jobs (migration 086; owner direction 2026-09-25). BUILT, build-verified, NOT YET PUSHED — run `npm run db:migrate` + `npm run db:verify` after deploy.** Spec rewritten: **`docs/refining-spec.md`**. Stations/cities no longer refine (Refinery sub-tab removed). `base_refinery` is CRAFT-ONLY (buy_price NULL, recipe Ti 120 / Cu 80 / Crystite 30 / Iron 100, `requires_tech = tech_refining`); its crafted quality drives the machine: +10% yield, +10 cap, 1.5× speed at Q100 vs Q50 (`MODULE_*` constants). Each fitted refinery is a LANE (up to 3 at Station tier). New `player_refine_jobs`: N units of one stack → floor(N × yield) at min(cap, Q + gain); duration `(30 + 0.8 × units) s ÷ speed` × Smelting (−5%/level, the skill now does something); 1 Fuel Cell per 100 units from cargo at enqueue (no credit fee); up to 6 jobs per lane; `starts_at`/`completes_at` fixed at enqueue (a job starts when the lane's previous job ends — no cron); cancel before start refunds input + fuel and pulls later jobs forward; collect while docked to cargo or depot (capacity checks); collecting completes **Grade Up**. `api/refining.js` rewritten: `/status`, `/quote`, `/queue`, `/jobs/:id/cancel`, `/jobs/:id/collect`. Client `RefineryPanel` rewritten as lanes + queue (1 s tick, 8 s poll) inside the Base tab. Onboarding chain reordered so **Homestead precedes Grade Up** (086 updates triggers/sort; the unpushed 085 was edited to match); Grade Up predicate = a collected job. Levers at the top of `api/refining.js`.

**Onboarding chain extension (migration 085). BUILT, syntax-checked, NOT YET PUSHED — run `npm run db:migrate` + `npm run db:verify` after deploy.** Six tutorial quests chained after "Coming Home" (`tutorial_collect_harvester`, sort 14–19): **Hired Gun** (accept any contract, 500 cr) → **Special Delivery** (complete a haul or fetch, 1.5k) → **Grade Up** (refine a stack, 2k) → **Something on the Band** (probe a signature, 2.5k) → **Dig Site** (investigate a pinned site, 3k) → **Homestead** (build a base, 5k). Completion is SERVER-SIDE inside the feature transactions (`completeQuestInTx` in contracts accept/deliver/turn-in, refining run, anomalies probe/resolve, bases build) — no client hooks, no cheat surface. `quests.js` gains `AUTO_SATISFY` predicates + `activateTriggered`: when a triggered quest activates and its condition is already met (player did it out of order) it completes immediately and the chain keeps walking; a link the pilot already holds as completed (backfill) is skipped over. **Existing pilots:** 085 backfills everyone who finished Coming Home — quests they already satisfied (any contract, a delivered haul/fetch, any probe, any investigation, a base) are inserted completed (no reward paid), and the first unsatisfied one is activated + pinned. Refining leaves no record so Grade Up is never pre-completed. Not done: a "New missions available" toast when the chain hands off.

**Shared asteroids + shared mining beams + pixel-art asteroids (2026-09-25). BUILT, build-verified, NOT YET PUSHED. Client + server, no migration.** (1) **Asteroid events:** `/resources/asteroids/mine` now emits `asteroid:update {id, depleted, remaining (by resource id), by}` to the presence room `presence:system:<procedural id>` after commit (procedural id resolved from the rock's system_id; Sol UUID → 'sol'); the list endpoint emits `asteroid:respawn {id, x, y, size, rotation, belt_body_id}` for every rock it just respawned. Client (SystemView effect via `socketBus.onSocketEvent`): depletion removes the rock for everyone and releases any of OUR lasers on it (toast "mined out by another pilot"); quantity ticks apply only to rocks we have scanned (the data-layer scan gate is preserved); respawns arrive as unscanned rocks. (2) **Mining beams over presence:** the 10 Hz pos packet carries `mining: [{i, a}]` (i = -1 flagship / wingman index in the fleet array, a = asteroid id; server caps 8 entries, validates types) built from `miningAssignmentsRef`; peers draw a dim green beam from that ship/wingman to the rock plus a spark dot. (3) **Pixel-art asteroids:** new `utils/asteroidRenderer.js` — per-rock seeded silhouette (3 harmonics), facets in rock space, 2–4 craters with lit lips, fixed upper-left light, 1-px dark outline + bright lit rim (contrast against the belt dust), scanned rocks get ore veins tinted by quality tier + a dashed tier ring; 14–44 px, 24-frame slow tumble (progressive 6 → 24), picked by spin fraction. Replaces the flat polygon; scan progress ring, mining lock rings, Nx badge and labels untouched.

**Pixel art v2: 64×64 portraits with two races, 32×32 item icons (owner: "up the resolution … a teeny bit too homogenous … Humans and humanoid cyborgs (Adeptus Mechanicus)"). BUILT, build-verified, NOT YET PUSHED. Client only.** `portrait.js` rewritten at 64 px: race per seed with a role bias (`ROLE_STYLE[role].cyborg`, e.g. Refinery Chief 70% cyborg, Flight Instructor 20%). **Humans:** 8 skin tones, 12 hair styles (short, long, bald, bun, cap, undercut, curly, ponytail, sidepart, buzz, beret, hood), 3 jaw shapes, eyes with iris + catchlight, brows, 3 noses, 5 mouths, beards (stubble, full, goatee), scars, freckles, tattoos, glasses, earrings, eye patch (pirates), role badge + rank stripes. **Cyborgs:** rust cowl (70%), steel face plating (half-left, half-right, lower, full skull, brow) with brass rivet seams, glowing optic lenses (one, both, mono; 4 colours), respirator grille, 1–3 cables from temple/jaw to collar, temple port, brass cog insignia, grey skin, Magos/Adept/Enginseer names with -Theta style suffixes and their own lines. Outline pass + rim light. `itemIcon.js` rewritten at 32 px on a small raster with poly/ring/disc primitives, 5-tone shading, 1-px outline, tier corner brackets + dashed edge, 4×4 quality gem; per-item variant = hash(id) + tier so a family's tiers and siblings differ (lasers: barrel length + 3 emitter styles; kinetic: magazine and sight; missile: fin sweep; shields: core size; plates: seam layout; reactors: pylon count). Portrait sizes: NPC tab, vendor and broker strips 64 px, cards 48 px; vendor rows 32 px icons. Names and lines are race-aware (`npcRace`). Checked via ASCII renders only — judge on the live build.

**Procedural pixel art for NPCs and items (owner picked "option 1: generators", 2026-09-24). BUILT, build-verified, NOT YET PUSHED. Client only.** `utils/pixelArt/portrait.js`: 32×32 NPC busts from a seed + role (species human/synth/exo, skin, head shape, hair or headgear — short/long/bun/mohawk/cap/helmet/visor/hood/bandana —, eyes, brow, mouth, scar/cyber-eye/beard/tattoo/glasses, role palette + collar), `npcName`, `npcLine`, `stationCast(systemId, bodyName)` (Quartermaster, Contract Broker, Dockmaster, Refinery Chief; cities swap in a Research Liaison), `questGiverFor(category)`. `utils/pixelArt/itemIcon.js`: 16×16 icons composed from a glyph (resource category ore/gas/bio/energy/exotic; weapon by damage type laser/kinetic/missile; shield/armor plate/reactor/engine/cargo/mining; utility subtypes dish/antenna/tube/compass/cross; base depot/flask/beaker; probe/fuel/harvester/warhead/sealed crate) tinted by rarity or slot colour, framed in the tier colour, quality gem bottom-right. `components/pixel/PixelArt.jsx`: `<Portrait seed role size>`, `<PixelItemIcon spec>`, `resourceIconSpec`, `moduleIconSpec`. Wired: Cargo window tiles, Crafting cargo tiles, the shared `ItemIcon` (Fittable Modules pane, vendor, tooltips), vendor module rows, a Quartermaster strip atop the Vendor tab, the station **NPCs** tab (was a stub) now lists the port's cast with lines, the Contracts board shows its Broker and named-elite bounties show the pirate's face, Missions cards show the quest giver (tutorial → Flight Instructor, main → Fleet Commander, side → Guild Contact, faction → Faction Envoy), the broker, or the pirate. All cached data URLs, pixelated scaling. Next (same approach): procedural hull sprites for new ships (needs care around the SystemView rotation conventions, pitfall #3) and base tier visuals (tint the station grids).

**Planet window uses the pixel-art planet (owner request 2026-09-24). BUILT, build-verified, NOT YET PUSHED. Client only.** New `components/system/PlanetSprite.jsx`: a self-contained animated sprite (own 30 fps rAF, re-renders only itself) built from the same `planetRenderer` sheets SystemView uses — rotation sheet + shade mask (star at upper-left by default) + emissive pass, rings/atmosphere included; `planetBaseColor(body)` mirrors SystemView's PLANET_TYPES first swatch for procedural bodies. `PlanetBanner` shows it (72 px) in place of the old gradient orb for planets; stations keep the old chip. Reusable anywhere a planet needs drawing (fleet/galaxy panels, base window).

**Activity feed moved into the chat panel (owner: "there has to be a better place … probably the chat window as its own tab"). BUILT, build-verified, NOT YET PUSHED. Client only, no migration.** The top-center ActivityTicker strip is no longer mounted (component kept in `components/activity/ActivityTicker.jsx` for its exported `formatEvent` / `formatAge`). `ChatPanel` gains an **Events** tab (System / Global / Corp / Events): newest-first galaxy feed from the same `utils/activity.js` buffer, sender names click to profile, per-tab unread badge counts live events while collapsed or on another tab, input row hidden on that tab. HUD stack is now fleet readout (top:38) → pinned mission tiles (**top:100**, was 132). Your OWN achievements still surface as toasts, so nothing personal is lost by tucking the feed away. Memory `feedback_fleet_status_readout.md` updated (the "ticker must never be obscured" rule is retired).

**Contracts folded into the Missions board (migration 084). BUILT, build-verified, NOT YET PUSHED — run `npm run db:migrate` + `npm run db:verify` after deploy.** Owner: "make sure hauling, fetch, and bounty are wrapped into the existing quest/mission board … we may need a better method to sort and filter". `QuestLogWindow.jsx` rewritten as ONE board: story quests + contracts in a unified list with **filter chips** (All / Story / Hauling / Fetch / Bounty), a **sort menu** (Priority = pinned → soonest deadline → story order; Time left; Reward; Tier; System; Newest), a **search box** (title / destination / system), and Active / Completed tabs (contract history = last 14 days, delivered/failed/expired/abandoned labelled). Contract cards carry type + tier + contested/rush pills, destination, live progress (freight aboard / have N/M / kills N/M), reward + time-left badges, and the actions: **DELIVER / TURN IN / COLLECT** (enabled when docked at the destination — `store.dockedBody.name` + `currentSystem`; the server still validates), **ABANDON**, **PIN**. New `player_contracts.pinned` (default TRUE) + `POST /contracts/:id/pin`; HUD tiles now show pinned contracts only (same rule as pinned quests); `/contracts/mine` returns 14 days of history (LIMIT 80). Store: `allContracts` + `setAllContracts` + optimistic `pinContract`; the poller feeds both lists; opening Missions bumps a refetch. The station **Contracts** tab is now the OFFER board only (a one-line "N active · OPEN MISSIONS" summary replaces its My Contracts list). Not done: a Missions toolbar badge; a "Board" tab in Missions showing the docked station's offers (kept at the station on purpose — offers depend on where you are).

**Anomalies & archaeology v1 — cosmic signatures (migration 083). BUILT, build-verified, NOT YET PUSHED — run `npm run db:migrate` + `npm run db:verify` after deploy.** Spec: **`docs/anomalies-spec.md`**. Loop: detect → probe → pin → fly → investigate. `game/anomalies.js` (pure): 0–3 sites per system per DAILY bucket from `hash(system, bucket)` (count by region tier; types Gas Pocket ☁ / Derelict ⚓ / Data Vault ▣ (T2+) / Relic Cache ◈ (T3+); position 500..outer-orbit×1.15; `guarded` 0→90% by tier), `estimateFor` (radius 700 × (1 − cycles/needed), per-pilot jitter), `rollRewards` (server-side from `hash(pilot, system, site, bucket)`: gas by tier at Q60–85, derelict credits + module drop 60% + salvage skill, vault RP 60→1400 + Data Analysis, relic exotics + module up to tier+1 at 35% + Relic Analysis ×2 with Xenoarchaeology; T4+ relics need Xenoarchaeology). `api/anomalies.js`: `GET /system/:id`, `POST /probe` (needs a fitted **Signature Probe Launcher** anywhere in the active fleet, pitfall #15; cycle = module `probe_cycle` 20s × (Survey Probing + Astrometric Acquisition); 3 cycles to pin, 2 at Rangefinding +20%; server-enforced cooldown per site), `POST /resolve` (within 90 units of the pinned position; guarded → `buildAmbushFleet` + new `registerUserAmbush` export in combat.js so loot claims validate). Progress per pilot per bucket in `player_anomaly_progress`. 083 adds the launcher module (+item twin, recipe; research `tech_signature_analysis`, Society T2) and `tech_xenoarchaeology` (T3); rewrites the 7 Astrometrics/Exploration skill descriptions that now do something (contracts unchanged; Skills window marks them wired). Client: toolbar **🔭 Signals** window (`components/anomalies/AnomaliesWindow.jsx`: PROBE with countdown / FLY TO (autopilot target type `anomaly` → SystemView synthesises a static virtual body, asteroid arrival rule) / INVESTIGATE within range); SystemView draws pinned sites + estimate circles from `store.anomalySites` and spawns a returned raider fleet from `store.pendingAmbush` (merges the new fleet's pools only); ticker `anomaly_resolved`; reset-account wipes progress. Not built: hacking mini-game (coherence contracts read as flat bonuses), site hazards, a deeper launcher tier, blueprint rewards, galaxy-map signature counts.

**Player bases Phase 1 (migration 082). BUILT, build-verified, NOT YET PUSHED — run `npm run db:migrate` + `npm run db:verify` after deploy.** Spec: **`docs/bases-spec.md`** (implements the STATUS "player-owned orbital stations" Phase 1 design; open questions settled: manage while DOCKED at the planet; depot is its own inventory; bulk-pay + real-time build timer; slots only grow). `player_bases` (one per pilot per body; cap 1 + `pln_interplanetary` level; kind surface|orbital — flavor only in Phase 1; tier 1–3 = Framework/Outpost/Station with 1/2/3 slots, timers 10/45/180 min, credit + bulk resource costs in `TIERS`; `fitted_modules` JSONB like ships) + `player_base_inventory` (depot stacks, stat-tuple identity). Base modules = `module_types` rows with `slot_type 'base'` (+ item_definitions twins + recipes, pitfall #19; vendor-buyable, research-gated): **Cargo Depot** (1500 units), **Base Refinery** (`refining.js` treats any planet with your fitted refinery as a fee-free refinery, +5% yield), **Research Lab** (+0.5 RP/min via `baseRpPerMin` folded into `effectiveRpPerMin`). Gates: `tech_base_construction` (Industry T2) build + modules, `tech_base_expansion` (T3) tiers 2–3, `pln_cc_upgrades` ≥ 1. `api/bases.js`: `/mine`, `/here`, `/build`, `/:id/upgrade|fit|unfit`, `/:id/depot/deposit|withdraw` — all docked-only (presence). Reset-account wipes bases; `db:verify` checks 082. Client: **Base** tab on every planet window (`components/base/BaseTab.jsx`), 🏠 galaxy-map marker + "Your Base" row, ticker `base_founded`. Phase 2 (not built): defense + raids, med bay, haulers, market post, sensor array, claims; surface vs orbital divergence.

**Refining v1 — quality refining at stations/cities (migration 081). BUILT, build-verified, NOT YET PUSHED — run `npm run db:migrate` + `npm run db:verify` after deploy.** Spec: **`docs/refining-spec.md`**. `api/refining.js` (`GET /quote`, `POST /run`): one resource stack in, `floor(N × yield)` units out at `min(cap, Q + gain)` (every stat shifted by the same delta), fee `ceil(N × base_price × 0.15)` up front; yield 0.65 + Processing skills (reprocessing_yield_pct, metal_refining_pct on ores, common_ore_refining_pct on commons — the catalog contracts were inert until now) capped 0.92; gain 8 (+4 with Deep Refining); cap 75 (95 with Deep Refining) +1 per Metallurgy level on ores. Gated by new research `tech_refining` (Industry T2, 450 RP, after Advanced Mining) and `tech_deep_refining` (T3, 1500 RP); docked at a station or city (presence → `resolveBodyId` → body_type station / has_city). Output merges via `addResourceStack`. The spec shows refine-and-vendor is a loss even at max skills (fee + unit loss beat the Q90 = 2.56× vendor curve) — refining is for crafting quality and contract floors. Client: **Refinery** sub-tab on stations AND cities (`components/refinery/RefineryPanel.jsx`: stack list, units-in slider, live quote, REFINE; locked state deep-links to the research node); Skills window marks the three Processing contracts as wired. Not built: tier refining (commons → rare-grade products as new resource rows via a `refining_recipes` table — waits for recipes that consume them), timed batches, a player-station refinery module.

**Bounty contracts (migration 080). BUILT, build-verified, NOT YET PUSHED — run `npm run db:migrate` + `npm run db:verify` after deploy.** Board now 12 offers per port: 7 hauls, 3 fetches, **2 bounties** ("Destroy N Tier T+ pirate ships"; 30% flagship-only × 1.8 pay; at T4/T5 ports 25% a named elite — Orsk 45k / Vask 110k). Pay `BOUNTY_PER_KILL` {700, 1.6k, 4.2k, 9.5k, 19k} × kills, deadlines 2–5 h. **Verified kills:** `buildClaimIndex` entries now carry `tier`; `/combat/claim-loot` calls `progressBounties` (exported from `api/contracts.js`) after a validated salvage — one UPDATE advances every active bounty the kill satisfies (`target_tier` ≤ kill tier, `target_flagship` ⇒ flagship, `target_template_id` ⇒ exact template) and the response carries `bounties` for the client toast ("🎯 … 2/3", "bounty complete"). So a kill only counts when its wreck is salvaged — by design (the salvage is the server's kill record). 080 adds `target_tier`, `target_template_id`, `target_flagship`, `progress`. Turn-in (`/:id/deliver` at the origin) pays × Negotiation when `progress ≥ cargo_volume`. UI: 🎯 rows in the Contracts tab + HUD tile with kills N/M, COLLECT BOUNTY button. Not done: promoting player-posted bounties (060) to the same hook; ambush-fleet kills count too (they carry tier).

**Contract framework polish (no migration). BUILT, build-verified, NOT YET PUSHED — ships with 078/079.** (1) **Contested hauls:** 25% of tier ≥ 2 haul offers carry `contested` (pay × `CONTESTED_PAY` 1.5, badge). `/combat/enter-system` runs `maybeAmbush`: for the pilot's active hauls, regenerate the offer from its key (`offerByKey(key, {anyBucket:true})` — boards are deterministic per bucket, so old keys still resolve), and if contested + this system is on `pathBetween(origin, dest)` (not the origin) + not yet ambushed, append a per-user raider fleet from `buildAmbushFleet` (enemyManifest.js; same tier, ids `ambush_<contract8>_n`, `fleet.ambush = true`) to the manifest + `manifest.ambush {contract_id, cargo_label}`. Client `hydrateEnemies(manifest, systemId, {ambushAt})` places ambush fleets 320 units from the arrival point in state `chase`; SystemView toasts "⚠ AMBUSH". Loot claims check `ambushIndexByUser` after the system claim index. One ambush per contract (in-memory). (2) **Freight at stake:** enter-pod ejects all `sealed_cargo` stacks via new `ejectItems` into the wreck (`contents.items`); wreck claim restores them (`items_awarded`, contract id intact); `/contracts/mine` returns `freight_units` for hauls and the panel/HUD say "freight lost — reclaim your wreck". (3) **HUD:** store `activeContracts` + `contractsVersion`/`bumpContracts`; `ContractsPoller` in GameFrame (60 s + on bump); PinnedQuestsOverlay renders `ContractTile`s under the pinned quests (30 s countdown tick); GalaxyFlightView shows 📦 on delivery systems. Levers: `CONTESTED_CHANCE`, `CONTESTED_PAY`, `CONTESTED_MIN_TIER` (contracts.js), ambush distance 320 (enemyManifest.js client).

**Find-resource ("fetch") contracts (migration 079). BUILT, build-verified, NOT YET PUSHED. Ships with 078 — run `npm run db:migrate` + `npm run db:verify` after deploy.** Same board, same table: each port adds `FETCH_SIZE` (3) offers after its 7 hauling offers (board size 10): "Bring N × Resource at Q≥floor, turn in here" (dest = origin). `game/contracts.js`: resource pool per tier by vendor base-price band (T1 ≤30 cr commons … T5 exotics), quantity bands 60–150 / 100–250 / 120–300 / 20–60 / 15–40, quality floor none / 40–55 / 50–65 / 55–70 / 65–80, pay per unit = `resourceSellPrice(base, floorQ) × FETCH_PREMIUM` {1.6, 1.8, 2.0, 2.2, 2.5} (always beats mining-and-vendoring the same ore), deadlines 90–240 min. The generator needs the resource catalog — `api/contracts.js` `ensureCatalog()` loads `resource_types` once per process into `setResourceCatalog` (generator stays pure). 079 adds `player_contracts.fetch_resource_type_id` + `fetch_min_quality`. Accept stores no item; `/board` and `/mine` return `have_qualifying` (units at avg quality ≥ floor); turn-in (`POST /:id/deliver` at the origin) consumes qualifying stacks lowest-quality-first, refuses partials with "you have N", pays × Negotiation, ticker says "supplied N X to Station". Client: ⛏ rows with "have N/M" and a TURN IN button when docked at the origin with enough. Dry run: avg reward T1 1.3k · T2 10k · T3 31k · T4 32k · T5 37k (T4/T5 need exotics at Q60–80, so the ore itself is the hard part).

**Contract board v1 — hauling contracts (owner direction 2026-09-22: "more to do than mining"; must pay a little more than mining). BUILT, build-verified, NOT YET PUSHED. Migration 078 → run `npm run db:migrate` then `npm run db:verify` after deploy.** Spec: **`docs/contracts-spec.md`**. Procedural + deterministic + server-authoritative: every STATION ("port"; Sol = Luna Station; 148 ports in 91 systems, all gate-reachable) rolls 7 offers from `hash(system, station, 4-hour bucket)` in `server/src/game/contracts.js` — same board for every pilot, refreshes on the bucket, offers are never stored (an accepted offer is regenerated from its key and validated). Pay = `volume × RATE[tier] × (1 + 0.25 × gate hops) × DANGER[worst region tier on the BFS path] × (rush 1.4)`, RATE ≈ 22–32 cr/unit (nearly flat — volume and distance carry the scaling), DANGER 1.0–2.1; deadline `8 + 5 × hops` min (rush × 0.6). Dry run averages: T1 2.2k · T2 8k · T3 32k · T4 88k · T5 210k per run (first pass with 0.4 hop pay + 3.8 danger hit 1.4 M at T5 — keep multipliers soft). Tier bands: volume 40–80 / 100–200 / 250–450 / 500–900 / 900–1500 cargo units (T3+ needs real hulls / fleet cargo), hops 1–2 / 2–3 / 3–4 / 4–6 / 5–8. **Player scaling:** `trd_contracting` re-purposed (078 sets its bonus to +1/level): contract tier cap = 1 + level, active cap = 2 + level; `soc_negotiation` (`mission_reward_pct`, +5%/level) now wired to the payout. **Data:** `player_contracts` (accepted rows only, status active/delivered/failed/expired/abandoned; unique live key per pilot) + `sealed_cargo` item (1 volume per unit, quantity = parcel volume, `item_data.contract_id`; priced 0 / refused by `/sell-item` and hidden from the Sell tab; NOT ejected into wrecks in v1 — items aren't). **API** `/api/contracts`: `GET /board` (docked port via presence → `resolveBodyId` → celestial_bodies→star_systems.procedural_id), `GET /mine` (lazy-expires overdue contracts and deletes their freight), `POST /accept {contract_key}` (docked at origin, tier ≤ cap, active < cap, cargo room), `POST /:id/deliver` (docked at dest, full stack present, before deadline → pays × Negotiation, logs `contract_delivered` to the ticker; late/short → failed, freight removed), `POST /:id/abandon`. Reset-account wipes `player_contracts` (pitfall #17); `db:verify` checks 078. **Client:** station **Contracts** sub-tab (`components/contracts/ContractsPanel.jsx`; stations only, not cities): My Contracts (time left, DELIVER HERE when docked at the destination, Abandon) + the board (tier badge, cargo, destination, hops, danger, deadline, reward, Accept disabled with the reason); galaxy map gets a green 📦 marker on destination systems + a "Deliveries Here" row; activity ticker renders deliveries. **Next on this framework:** fetch contracts (bring N of X at ≥Q), verified bounty contracts (kill signal = the loot-claim record), contested hauls (ambush fleet on the route), failure penalties / port reputation. Levers: constants at the top of `game/contracts.js`.

**Quality tier "Refined" renamed to "Fine" + one quality palette (owner 2026-09-22). BUILT, build-verified, NOT YET PUSHED. Client + server, no migration.** "Refined" was the 41–60 quality band label and would collide with the planned refining mechanic. Renamed in the single source `data/resources.js` `QUALITY_TIERS` (key `FINE`), the SystemMapWindow bucket, and the server mirror `getQualityTier` (api/resources.js + the unused game/resources.js copy). The cargo/crafting tile border maps were hand copies with DIFFERENT greys for Impure/Standard — now derived from the shared `QUALITY_TIER_COLORS`; Impure/Standard are greys (#666e78 / #b0bcc8) so neither reads as the white "common" rarity color. **The "blue resource but Refined" report was not a mapping bug:** in scan cards the resource NAME is colored by RARITY (white common / blue rare / purple exotic) and the pill is QUALITY — two scales on one row. The ground-scan card now prints a RARE / EXOTIC tag beside the name so the blue is explained. Quality tiers are computed at read time (never stored), so no data changes.

**Galaxy map: your harvesters per system (owner request 2026-09-22). BUILT, build-verified, NOT YET PUSHED. Server + client, no migration.** New read-only `GET /harvesters/mine` (api/harvesters.js) lists every harvester the pilot owns joined through celestial_bodies → star_systems.procedural_id, grouped `by_system[procId] = {system_name, count, planets:[{body_id, body_name, harvesters:[{harvester_type, resource_name, hopper_quantity, storage_capacity, fuel_remaining_hours, status}]}]}` with hopper/fuel/status computed live via `updateHarvesterState` but NOT persisted (the planet tab + collect/refuel own persistence). Client `harvesterAPI.mine()`; GalaxyMapWindow fetches it on every open: amber `⚙N` marker at the upper-right of any system dot where you have harvesters, a "Your Harvesters" row in the info panel, and a per-planet list beneath (resource · hopper % · fuel h · status; active green / full cyan / idle red — idle = out of fuel or deposit depleted). Not built: the same marker in GalaxyFlightView; a HUD-level "harvester full" alert.

**Vendor sell prices rebuilt (owner: "a lot of stuff is just sell to vendor for 5 credits"). BUILT 2026-09-22, build-verified, NOT YET PUSHED. Server + client, no migration.** New **`server/src/lib/pricing.js`** is the single source of truth; `GET /resources/inventory` now returns `sell_price` per stack/item and the Sell tab displays that (the old client-side guess showed 5 cr for every module because `item_data.module_type_id` never existed). Rules: **(1) vendor-stocked goods sell back at 90%** (`VENDOR_SELLBACK`) — supplies (probe 45 / adv probe 135 / fuel cell 90 / warhead 27) and modules with a `buy_price` whose `item_data.source` is `'vendor'` **or missing (legacy items = generous reading)**; **(2) crafted / looted / craft-only modules** = recipe materials at the vendor's resource rate × `TIER_MARKUP` {1:0.85, 2:1.2, 3:2.0, 4:3.2, 5:5.0} × quality (linear, Q90 = 1.8×) — so a crafted T1 Q50 Armor Plating is 138 cr against 163 cr of ore (small loss, as specced), a Q50 Plasma Beam Lance 10.2k, a looted Q95 lance 19.3k, a Q70 Quantum Torpedo Launcher 6.8k; **(3) resources** keep base_price × 0.5 at Q50 but on a steeper curve (`RESOURCE_Q_EXP` 1.6: Q90 = 2.56×, Q100 = 3×, Q30 = 0.44× — Iron 2/5/9/13/15 cr at Q30/50/70/90/100; Void Essence 166/375/642/960/1137). **Provenance stamps** (`item_data.source`): `/buy-module` + starter kit → `'vendor'`, `/craft` (+ dev cheat-craft) → `'crafted'`, wreck + elite-drop inserts (`insertModuleItem`) → `'loot'`; fit copies `source` onto the fitted slot and unfit restores it. Recipes are read from `crafting_recipes.ingredients` (fallback `module_types.recipe`); items with no recipe, no buy_price and no supply row hit `FLOOR_BY_TIER`. Catalog cached 5 min (`resetPricingCatalog()` if you edit prices live). Refined / higher-tier resources later = new `resource_types` rows with their own `base_price`; nothing here changes. ⚠ Design note: a vendor-bought T1 module still sells for 90% of its 1200 cr price while the same module crafted from ~160 cr of ore sells for ~140 — that is the point of the provenance stamp (no craft→vendor credit loop). Side effect: newly crafted items won't merge into pre-existing crafted stacks once (the stack key is the full item_data). Levers: all the constants at the top of `pricing.js`.

**Enemy wingmen teleporting onto the player (owner report 2026-09-22). FIXED, build-verified, NOT YET PUSHED. Client only.** Cause: enemy wingmen reuse the player fleet's uncapped exponential lag-follow (`WINGMAN_LAG_RATE` closes ~90% of ANY distance in ~600 ms). Harmless for V slots beside the flagship, but the Phase 4a flank slots sit around the PLAYER at orbit range — the instant the flagship entered `attack`, every wingman's slot jumped hundreds of units and the lerp snapped them there while the flagship kept flying at its real speed. Fix in the follower block of the enemy AI (`SystemView.jsx`): (1) per-frame movement toward the slot is capped at `enemy.speed × 1.2 × delta` (a little over the leader so they still catch up on turns); (2) flank slots only activate once the flagship itself is within 1.5× its orbit range of the player — until then wingmen hold the V, so the fleet arrives together and then fans out. Lever: the 1.2 speed factor and the 1.5 proximity factor.

**30 fps sprite animation + progressive baking (owner: "animations are still stuttery"). BUILT 2026-09-21, build-verified, NOT YET PUSHED. Client only.** Before: planets 2.4–9.6 fps (48 frames × spin), stars 12, beam 14, gate 12, warp 14 — every loop was frame-starved. Now every loop targets ~30 fps: stars 80 frames / 2.7 s, pulsar beam 64 / 2.3 s, gate 30 / 1.0 s, warp 26 / 0.9 s, planets get `framesFor(size) = clamp(ceil(30 / spinRate), 48, 200)` (150–200 in practice; capped by canvas width — a ringed giant's sheet is 128 × 200 = 25 600 px wide, under the 32 767 browser limit). New `utils/spriteBake.js`: `bakeSheet` (sync), `bakeSheetAsync` (4 frames per `setTimeout` tick, never stalls a game frame), `progressiveSheet(cache, key, quick, full, spec)` — returns a 16-frame quick sheet synchronously on first sight, bakes the full sheet in the background and swaps it into the cache (smoke test: all 9 planet types + 2 stars + beam land in < 1 s total). Planets, stars and the beam use it; gate / warp / stations stay sync (≤ 4 ms). **Frame picks in SystemView are now loop FRACTIONS** (`((time / loopSeconds) % 1) * sheet.frames`, planets `(time * spinRate) % 1`), so the quick→full swap is seamless and no code depends on a frame constant except gate/warp. Flicker effects were per-frame (faculae, gate sparkle, warp glitter) and would have buzzed at 30 fps — now change every 4th frame / 16 states per star loop. Station light blink unchanged (2 frames, 1.5 Hz). The remaining ceiling is the 60 fps game loop itself; going past 30 fps would double sheet widths for little gain.

**Pixel-art v2 tuning (owner playtest: "stations look flat", "animations stutter"). BUILT 2026-09-21, build-verified, NOT YET PUSHED. Client only.** (1) **Planet lighting decoupled from texture**: `getPlanetSheet(body, color)` now bakes an UNLIT 48-frame texture once per planet (materials at their bright step); `getShadeMask(size, hasRings, lightIdx)` is a one-frame disc of multiply factors (0.36/0.58/0.8/1.0 + checker dither) shared by every planet of that pixel size, composited over the texture with `mix-blend-mode: multiply` in SystemView. No re-bake when the star-facing light index changes (the old per-light-direction 24-frame sheets caused a hitch each time the terminator ticked). Spin raised to 0.05–0.2 rev/s → ~2–10 fps of rotation depending on size; frames 24→48 so each step is 7.5°. Glow pass (same day): `getPlanetSheet` also bakes an `emissiveDataUrl` sheet holding only self-lit pixels (lava fissures, exotic bright bands — types without emissives return null); SystemView draws that frame ABOVE the shade mask, so fissures burn through the night side. (2) **Station shading**: upper-left light — edge pixels with open top/left get ×1.28, open bottom/right ×0.66, interior rows alternate ×1.0/×0.88 (panel seams), 1-px dark outline around the silhouette; windows/nav lights/accents stay emissive. (3) Frame counts doubled: gate 12 @ 12 fps, warp 12 @ 14 fps. (4) **Stars rebuilt (v3)** after "hokey / low fps" then "tendrils are unnecessary, make it wow": tendrils REMOVED on all stars; disc up to 64 px with 6-step ramp, drifting convection cells, limb faculae, drifting sunspot groups (umbra/penumbra) on red/orange/yellow stars and bright plages on hot ones; outward-fading ejecta particles kept; **pulsar beam is now a pixel sprite** (`getPulsarBeamSheet`, two-lobed jet widening toward the tip, dithered falloff, half-turn 32-frame loop @ 14 fps) replacing the rotating SVG rect. Earlier v2 notes: granulation is now 26 seeded convection cells that drift on small closed loops (phase = frame → seamless 32-frame loop, features MOVE instead of re-rolling each frame); smooth limb darkening + dithered 5-step ramp (rim-dark → white core); corona = tendrils from two travelling waves around the limb plus sparse ejecta that ride outward and fade; black-hole streaks advect around the ring. 32 frames @ 12 fps (2.7 s loop).

**Pixel-art stars, stations, jump gate, warp point. SHIPPED 2026-09-21 (pushed by owner). Client only.** `utils/structureRenderer.js` + a shared `SpriteFrame` in SystemView (nested `<svg viewBox>` frame picker). **Stars:** 48-px disc, radial 4-step ramp from the type's core/mid/outer colors, rim granulation + 12 corona spikes that breathe across 8 frames (~6 fps); the SVG blur glow stays behind the sprite; neutron keeps its rotating pulsar beams; **black holes** get a rotating tilted pixel accretion ring (hot inner edge → orange → dusty gaps, hidden behind the hole on the far half) around a black disc with a 1-px lensing ring. **Stations:** 10 hand-authored 17×17 pixel designs (ring, hub+pods, cross truss, cylinder+arms, relay dish, spire tower, dock bar w/ hangar pad, asteroid outpost, twin rings, industrial block w/ cranes) in `STATION_DESIGNS` (char grids: h/d/l hull shades, w windows, r nav lights, a cyan accent, g green pad); design = hash of the station id (stable per station); windows/nav lights blink on 2 frames; drawn ~3.2× body.size (≥24 world units) — the old hexagon was tiny. **Jump gate** (one design everywhere): grey ring + 4 green-tipped pylons + 3 rotating green arcs + core, 6 frames. **Warp point** (one design): purple 3-arm spiral vortex, bright core, 8 frames. All cached after first generation (≤23 ms). Target rings and labels untouched. To add a station variety: append a 17-row grid to `STATION_DESIGNS` (ids hash mod the count, so existing stations may re-pick — add before launch or accept the reshuffle).

**Pixel-art planets (owner style pick: pixel art, "fine" resolution). BUILT 2026-09-21, build-verified, NOT YET PUSHED. Client only.** `utils/planetRenderer.js`: per planet a sprite SHEET (24 rotation frames) is generated once from the body seed — pixels sphere-mapped to lon/lat, sampled from a per-type texture (gas giant bands + storm oval; terran continents/shore/highland + polar caps; ocean islands; rocky/barren craters; desert dune bands; ice cracks; lava fissures (emissive); exotic swirl), lit by a light direction quantized to 16 angles (the lit side faces the star — `lightIndexFor(x,y)`), reduced to a 4-step ramp per material with checker dithering at step boundaries; habitable worlds get a cloud layer spinning at 2×; rings baked as a tilted pixel ellipse (far half hidden, near half in front, with a gap); atmosphere = 1–2px rim brighter on the lit side. Resolution 24–64 px across (`spritePixels(size)`), same canvas→dataUrl→`<image image-rendering:pixelated>` pipeline as the ship sprites. Scene cost: pick a frame (`time × spinRate(size)`) inside a nested `<svg viewBox>` — no per-frame drawing; sheets cached per (planet, light index), ~10 ms each on first use. The old flat circle / top-left highlight / flat ring ellipse are gone; lava/exotic blur glow stays behind the sprite. Tweak knobs: `MIN_PX/MAX_PX`, `FRAMES`, `LIGHT_DIRS`, shade thresholds in `getPlanetSheet`, palettes in `TEXTURES`. Not done: star sprite (next), System Map dots unchanged, Sol planets use their `color` as the base material.

**Combat Phase 4b — stakes: player wrecks + elite drops. SHIPPED 2026-09-21 (deployed, 077 applied). The combat arc (Phases 0–4) is complete.** Closes the combat arc (plan §C Phase 4). **Player wrecks are server-created only** (`src/lib/wrecks.js`): `/fitting/enter-pod` now takes the death `x/y/system_procedural_id` and ejects the flagship's fitted modules (with their instance quality) plus **`EJECT_CARGO_FRACTION` = 50% of every resource stack** (floor per stack; cargo is fleet-pooled so the flagship popping = the hold breached) into a `wrecks` row (`source 'player_flagship'`, 30-min TTL); `/fitting/lose-ship` ejects a dead wingman's modules only (`'player_wingman'`). Old clients that send no position just lose the ship as before. **`POST /resources/wrecks/spawn` is deleted** (the audit's client-driven credit mint); `/wrecks` list adds `source`; `/wrecks/claim` (first-touch, transactional) now also merges ejected resource stacks back into cargo (`addResourceStack`, stat-tuple merge) and returns `resources_awarded` + `ship_name`. **Elite drops:** `enemy_templates.loot_table` (077 seeds Dread Captain Orsk → Quantum Torpedo Launcher Q70-85 always + Alloy plating 50%; Admiral Vask → Plasma Beam Lance Q80-95 always + Void Barrier 50% + Void Drive 35%; Titan → Mass Driver 15%) rolls server-side in `/combat/claim-loot` (manifest claimIndex carries `templateId`) straight into cargo; response adds `items`. **Client:** enter-pod / lose-ship send the death position (wingman position captured before its slot clears); server wreck polling is back (5s, merge-by-id, local pirate wrecks untouched); own-death response pushes the wreck immediately with a toast ("Your wreck holds N modules and M cargo stacks — 30 min"); pickup loop branches server wrecks → `wrecksAPI.claim` (toast lists modules/cargo), pirate wrecks → `claimLoot` (+ "★ Salvaged: …" toast for elite drops); **contested rule**: no salvage while a hostile in attack/chase is within 220 units (throttled toast); player wrecks render red-tinted with "WRECK · <ship>" + "N mods · M cargo · Xm" countdown. Anyone in the system can take a wreck — that's the point. Not built: NPC pirates physically looting wrecks (the "contest" is the no-salvage-under-fire rule); wreck ownership/priority window for the victim (first-touch is deliberate for now). Levers: `EJECT_CARGO_FRACTION`, `WRECK_TTL_MIN`, `CONTEST_RANGE`, the loot tables.

**Asteroid telemetry scanners (migration 076). SHIPPED 2026-09-20 (deployed, 076 applied — first run failed on the item_definitions FK, fixed in place).** Owner design: telemetry decides which asteroids APPEAR in the System Map's Bodies panel (click-to-fly like planets); scanning still decides what you know (quality/minerals stay scan-gated at the data layer — `/resources/asteroids` only sends contents for rocks THIS player scanned, so no server list change was needed). Three utility modules with `stats.telemetry_tier`: **T2 Asteroid Telemetry Array** (tier 1: scanned rocks, quality + distance; vendor 6000 cr + recipe), **T3 Deep Telemetry Array** (tier 2: + every rock inside fleet sensor range, minerals column for scanned rocks; craft-only), **T4 Systemwide Telemetry Grid** (tier 3: every rock in the system + mineral filter box; craft-only, Quantum Dust recipe). Research: `tech_ast_telemetry` (T2, 400 RP) → `tech_ast_telemetry_deep` (T3, 900 + Crystite) → `tech_ast_telemetry_grid` (T4, 15000 + Quantum Dust), off Sensor Array Networking. Fit gate: **Survey** skill (`ast_survey`) at tier−1 via a new `utility.telemetry` sub-family in `fitGates.js` (both copies). Client: SystemView's scanner-data push now carries `telemetryTier` (fleet-wide best, pitfall #15) + an `asteroidCatalogue` projection (id, pos, scanned, avg quality, minerals, distance); SystemMapWindow renders an "Asteroids · N" section under Bodies (green dot = scanned, grey = unscanned, "scan to reveal minerals" hint at tier ≥2), rows sorted by distance; clicking a row sets `autopilotTarget {type:'asteroid'}` — SystemView synthesises a static virtual body for the rock (orbitSpeed 0) so the existing intercept/approach math works, and arrival holds station beside it (no dock, no window). Autopilot footer shows asteroid targets. Follow-up: the T1 list doesn't refresh until the next scanner push (~5 frames) — effectively instant.

**Skill queue drag-and-drop reorder. SHIPPED 2026-09-20 (deployed). No migration.** Owner request. `POST /skills/queue/reorder { order: [positions…] }` (every current position once); validates per-skill level order stays ascending; if the head changes, the displaced head's live SP is banked exactly like removal and the new head starts now (from its own bank, if any); everything re-chains through `scheduleEntry`; an unchanged head keeps its exact row. Client: native HTML5 DnD on the queue strip (`SkillQueueStrip` — grab handle ⋮⋮, drop highlight), store `reorderSkillQueue` refetches and toasts "previous training progress banked" when the head moved; a rejected order snaps back to the server's.

**Two small pre-4b items. SHIPPED 2026-09-20 (deployed, 075 applied).**
- **Per-player harvester cap per planet.** Slots are shared planet-wide, so one pilot could fill a good world. `/harvesters/deploy` now caps a pilot at `HARVESTER_CAP_BASE (2) + pln_cc_upgrades level`, never above the planet's slot count (`getPlayerSlotCap`); `GET /harvesters/planet/:id` returns `my_slot_cap` + `my_slot_count`; the tab header reads "N harvester slots · your cap here 2 (reached)" and click-to-deploy stops offering slots at cap. The catalog's Command Center Upgrades (`max_planets_flat`, "+1 planet per level") is applied as +1 harvester slot per planet per level until colonies exist.
- **Field repair — Repair Nanite Hive (075).** T3 utility, vendor 12,000 cr, no research gate: stats `{hull_repair_per_sec:2, armor_repair_per_sec:1, repair_delay:8, combat_tuned}`. SystemView sums every fitted hive fleet-wide (pitfall #15) × quality × Fleet Support skill (`remote_rep_pct`, +5%/level — now wired) and repairs the pooled hull, then armor, once `repair_delay` seconds have passed since the last hit (`lastPlayerHitTimeRef`, game-time). Not while podded. The fleet readout label flips to "HULL⚕" while it ticks. Hull damage still persists + still costs credits at a station — the hive is slow (a 2000-hull capital pool takes ~17 min at one Q50 hive) so station repair stays the fast path.

**Combat Phase 4a — enemy behavior tiers + rally cap. SHIPPED 2026-09-20 (deployed). Client only, no migration.** Plan §C Phase 4 (behavior ladder + rally cap; stakes/wrecks = Phase 4b, not started). Behavior = `enemy.behavior` from `utils/enemyManifest.js` `behaviorFor()` — the template's `behavior_mode` overrides when it isn't the seed value 'aggressive', else by tier: **simple** (T1, the old AI) · **evasive** (T2: orbits at 1.6× range, flips orbit direction every 2.5–5s, `kite` state backs off to 1.4× attack range when fleet shield < 25% until it's back over 60%) · **coordinated** (T3: wingmen leave the V in combat and take flank slots spread around the player; the fleet calls the nearest idle fleet within 900 units) · **tactical** (T4: + one `regroup` per fight when pooled hull < 40% — flies home, shield regen ×4 there, hull never regens, re-engages at 90% shield / 12s; calls any idle fleet in the system; flees at 10%) · **elite** (T5: tactical + flagship signature move on an 18s cooldown — named elites fire an ALPHA STRIKE (every weapon ×3 projectiles at 1.5× damage, toast), other T5 flagships SHIELD SURGE +30% max shield). Attack range now follows the fleet's longest weapon (`max(150, range×0.9)`) instead of the flat 150. **Rally cap** `RALLY_CAP_BY_TIER` {1:1, 2:2, 3:2, 4:3, 5:3}: a fleet may switch patrol→chase (aggro, rally, or reinforcement call) only while the system has fewer engaged fleets than the cap; engaged = chase/attack/kite/regroup. Target-fit panel shows the behavior. All numbers in `BEHAVIOR_TUNING` + the constants under it in `SystemView.jsx`. Playtest watch-list: T2 kiting may feel like running away (raise `kiteShield` threshold to shorten), T4 regroup can read as "they left" (the 12s cap bounds it), alpha strike at Q85 lances is a real burst (~3× a normal volley).

**Galaxy map v2 — route planner + route autopilot. SHIPPED 2026-09-20 (deployed). Client only, no migration.** `utils/routePlanner.js` `findRoute()` = BFS by hop count over the same edges the drive can actually use (gate hop if connected and tier ≤ class+2; warp hop if inside the free-warp ring and tier ≤ class+1 — reuses `utils/warp.js`), gate hops preferred on ties, routes may pass through undiscovered systems (shown as "Unknown"). **Map:** selecting a system shows "🧭 N hops · X gate, Y warp" + the hop chain, a dashed cyan preview polyline (dotted segments = warp hops), and **Plot course** (closes the map). An active route draws solid green with the "hop i / N · ✕ cancel" row. **Autopilot:** store `plannedRoute {targetId, hops, index}`; SystemView's route-follow effect turns the current hop into `pendingJump` + an autopilot to the matching exit body (jump gate → instant jump on dock; warp point → galaxy flight with the existing pendingJump→galaxy-autopilot path); `enterSystem` advances the index on arrival at the expected hop, drops the route on a manual detour, and the last hop toasts "Route complete". Plotting from galaxy view works once you enter a system. Not built: re-planning when the fleet's drive changes mid-route (the next hop just fails with the normal warp-refused message); a HUD route strip outside the map (toasts per hop for now).

**Healing hulls — station repair + persisted fleet damage. SHIPPED 2026-09-20 (deployed, 074 applied).** Owner request (2026-09-17). Before: hull/armor lived only in client refs and snapped to full on every system change — a free heal, so nothing to repair. Now: **the pooled fleet hull/armor FRACTIONS persist per user** (`users.fleet_hull_pct/fleet_armor_pct`, 074; the combat model is one pooled entity, a per-ship split would be invented). SystemView `syncFleetDamage()` mirrors the fractions to the store and POSTs `/fitting/fleet-status` on dock/undock, system change, every 10s (when moved ≥1%), and after repair; the first `/fitting/fleet` read seeds the refs (reload / re-login no longer heals). **System change no longer refills hull/armor** — shields still recharge on the warp. Pod entry still resets (new hull). **Repair**: new 🔧 Repair sub-tab on every station/city (PlanetInteractionWindow `RepairTab`) shows both pools + cost; `POST /fitting/repair {body_id, hull_pct, armor_pct}` validates the body is a station or has_city, recomputes the fleet's max pools server-side (Σ base_hull + Σ armor_hp×Q, same rule as enemyManifest), charges `missing × rate` (hull 2 cr/pt, armor 3 cr/pt — `REPAIR_RATES`, also returned on `/fleet` so the panel matches), refills both to 1.0. Starter scout full repair ≈ 400 cr; capital ≈ 4k. The dead `/repair-cost` endpoint is gone. `/reset-account` resets the fractions. Not built: field repair module / repair skill (catalog has `eng_*`/`log_*` repair contracts to wire later); passive regen deliberately not added (plan: undercuts stakes). Lever: `REPAIR_RATES`.

**Combat redesign Phase 3b — warp-range travel gating. SHIPPED 2026-09-20 (deployed, 073 applied).** Plan B9 / spec §5. **Rules** (one table, `star-shipper/src/utils/warp.js`, mirrored in `server/src/game/warp.js` — edit both): *drive class* = the fleet's weakest best-engine tier (no engine on any active ship = 0; `fit-module` now snapshots `tier` on the instance, older fits fall back to the id convention); *free warp* (galaxy flight from the system you left) allowed when distance ≤ `WARP_RANGE_BY_CLASS[class]` (600/800/1050/1350/1700/2100, × (1 + `jump_range_pct`) from Jump Drive Calibration) AND target tier ≤ class+1; *jump gate* = instant jump to a gate-connected system, allowed up to tier class+2 ("the gate takes you one step deeper than you can fly"). **Gate network rebuilt** (`galaxyGenerator` step 2, server copy re-synced verbatim): the old Kruskal-ish loop stopped at 60% coverage and seeded islands — only 4 systems were gate-reachable from Sol and T4→T5 had zero gates. Now: Kruskal spanning tree over every tier-adjacent pair (|Δtier| ≤ 1, any distance — gates are instant), + ≥3 guaranteed frontier links per adjacent tier pair (the chokepoints), + 15% short redundancy links. Dry run: 200/200 gated, 236 links, 0 tier-skips, every system gate-reachable from Sol, T3→T4 and T4→T5 each have exactly 3 gates. **Galaxy gate layout changed** (positions/tiers/stars unchanged; jump-gate bodies may appear/disappear in a few systems). **Views:** SystemView — docking at a jump gate with a pending map target is now an INSTANT jump (no galaxy flight); warp point = galaxy flight as before. GalaxyFlightView — dashed free-warp ring around the departed system (soft wall: position clamped to the ring, outward velocity killed, autopilot cancelled, "WARP RANGE LIMIT" message); unreachable targets refused at click/arrival/Enter with the reason; hover panel shows In warp range / Reachable by jump gate / 🔒 reason. GalaxyMapWindow — ring on the current system, reachability row in the info panel, Fly To disabled outside the ring (tooltip says to use the gate), Jump button gated by gate tier with "Gate locked — …" text. **Server:** `POST /galaxy/visit` validates every entry against `last_system_id` (gate-connected OR ring+tier) → 403 `Out of warp range`; the store bounces the client back to the previous system with a toast on that error (never fires on a legit client). **Not built:** route planner / multi-hop path highlight (galaxy map v2 item) — with gates instant it's now purely UI. Playtest levers: `WARP_RANGE_BY_CLASS`, the +1/+2 tier ceilings, `MIN_FRONTIER_GATES`.

**Combat redesign Phase 3a — capability gating (skill gates). SHIPPED 2026-09-19 (deployed, 071 applied).** Plan B3: research gates crafting, skills gate flying. Single config **`server/src/game/fitGates.js`** (`GATE_CONFIG` is sent to the client in `GET /skills` → `gameStore.fitGates`; client lookup logic in `utils/fitGates.js`). (1) **Module tier gates** — `/fit-module` requires the family's operation skill at level (tier − 1) for T2+ weapon + shield-slot modules: laser → Small Energy Turret Operation, kinetic → Small Projectile Turret Operation, missile → Missile Launcher Operation, shields → Shield Upgrades, armor → Armor Layering (all existing catalog skills, all rank 1–2; T5 = L4 ≈ 30–60 h). T1 free; engines/reactors/mining/utility/cargo ungated; already-fitted modules grandfathered (only fit-time check). (2) **Hull class gates** — `/buy-hull` only: frigate/freighter → Frigate Command I, prospector → Industrial Command I, excavator → II, leviathan → III, capital → Capital Ship Command II; fighter/scout/shuttle free; owned hulls grandfathered. (3) **Fleet size** — cap = 2 + Fleet Command level (`cmd_fleet_command`, Spaceship Command, rank 4), max 5; enforced in buy-hull + activate-ship, reported by `GET /fitting/fleet` (`fleetCap`, `fleetCapMax`); **071 grants Fleet Command retroactively** to anyone flying 3+ non-pod ships at the covering level (L1/L2/L3 = 1000/6660/38696 SP, the server's exact cumulative math) so nobody loses a ship. UI: Ship Builder hull rows show `🔒 Requires X` + disabled; fittable-module tiles get a 🔒 badge + tooltip; slot drop pre-checks with the requirement in a flash (server 403 carries the same text); FleetWindow shows "train Fleet Command" while cap < 5. `db:verify` asserts nobody is over their cap. Server messages use the skill NAME + roman level. **Still to do in Phase 3:** warp-range travel gating + gate routing (B9) — touches the protected galaxy views; plan it as its own session with the galaxy-map route planner.

**Combat redesign Phase 2 — enemy template system. SHIPPED 2026-09-17 (deployed, 069 applied).** Per `docs/combat-redesign-plan.md` §C Phase 2 + settled point A4. Enemies are now assembled SERVER-SIDE from the player's own catalogs: new tables `enemy_templates` (id, name, tier 1–5, faction, hull_type_id → hull_types, role escort|line|flagship|elite, spawn_weight, loot_multiplier, is_elite, loot_table JSONB placeholder for Phase 4 drops) + `enemy_template_modules` (template, slot_type, module_type_id → module_types, quality_min/max). The three pirate-visual hulls became real `hull_types` rows (price NULL = never sold; base_hull 60/140/280). 18 seeded templates across T1–T5 incl. two named elites (T4 "Dread Captain Orsk", T5 "Admiral Vask"). **Server `src/game/enemyManifest.js`** rolls each system's fleets deterministically from the system seed (fleets-per-danger 0/2/3/3/4/4, size-per-tier 1–2/2–3/3/3–4/4, escorts 70% same-tier / 30% tier-1, T5 fleet 0 always the named admiral, 20%/10% elite flagship chance at T4/T5) and resolves every stat with the PLAYER's math (hull = base_hull; shield/armor = module hp × Q, untuned legacy modules use the HUD's flat 40/25; weapon dmg × Q, range × √Q; engine = `thrust ?? speed` exactly like recalcShipStats). `POST /combat/enter-system` now RETURNS the manifest (plus re-arms loot claims as before); `GET /combat/manifest/:systemId` read-only; dev-only `POST /combat/reload-templates` drops the cache after editing template rows. Loot: base 35–120 × √(hull/60) × per-tier mult (`LOOT_TIER_MULT`, tuned in a dry run to ±15% of the B5 targets: avg payout per full clear T1 1.6k · T2 3.1k · T3 8.9k · T4 15k · T5 24k; Sol 0.8k). Galaxy dry-run: 1745 enemies across 200 systems, max 20 per system, 31 empty. **Client:** `utils/enemyManifest.js` hydrates the manifest into sim objects (icon/displaySize/engine glow from shipRenderer); SystemView's spawn effect fetches on system entry (retries every 5s on failure, gives up on 404, drops stale responses on system change; no client fallback — an unreachable server = empty system); **enemies fire every fitted weapon on its own cooldown/range/damage-type** (`fireEnemyWeapons`; a Titan cycles lance + driver + torpedoes independently); new **designated-target fit panel** right of the fleet readout shows hull/HP/speed + every module with rolled quality and resolved numbers (inspectable fits). **DELETED:** server `pirateManifest.js` (the hand-mirrored RNG stream — pitfall #16 rewritten), client `generatePiratesForSystem` + all `PIRATE_*` catalogs + Sol spawn zones (now server data). Also removed 9 stray tracked `vite.config.js.timestamp-*` files + 2 `.tmp` files; `.gitignore` covers them. ⚠ Finding: player engines add 0 speed today — `recalcShipStats` reads `thrust ?? speed` but engine rows only carry `speed_bonus`. Enemy side mirrors that on purpose (parity); fixing it is a one-line change in `api/fitting.js` that must be mirrored in `enemyManifest.js`. Playtest levers: `FLEETS_BY_DANGER`, `FLEET_SIZE_BY_TIER`, `LOOT_TIER_MULT`, template quality ranges.
...
- **IN BUILD (the new combat core) — fleet-vs-fleet collective entities.** Full mechanic specced in `docs/combat-attrition-spec.md`. Build order F1→F4.
  ...
  - **F1.5 SHIPPED (build-verified) 2026-06-03** — enemy fleets now FLY in formation. Flagship = formation leader (runs the AI); wingmen mirror its combat state + lagged-follow a V-formation slot, reusing the player fleet's exact formation math (`WINGMAN_LAG_RATE` + slot decomposition + heading rule). A fleet of 1 has no leader → flies free. Held in combat too (leader maneuvers, wingmen follow + fire from slots). `buildFleets` assigns `formationOffset` from the shared `FORMATION_OFFSETS`. Forward note: selectable formations + role-based slotting (e.g. kinetic ships front-line to strip shields) are a later feature.
  - **F2 SHIPPED (build-verified) 2026-06-03** — enemy attrition. `buildFleets` precomputes each fleet's death order (escorts lightest-first, flagship last) + per-member hull thresholds `T_k = maxHull − Σh_1..k`. `checkFleetAttrition` (called after every hull-damaging hit) peels off any member whose threshold the pooled hull crossed: it explodes, drops its own loot wreck, and stops firing/moving (DPS decays as the fleet shrinks). Shield+armor are buffers — no ship dies while they hold. Flagship's death (threshold 0, last) destroys the fleet. Target-lock clears when the designated ship dies. No migration.
  - **F3 SHIPPED (build-verified) 2026-06-10** — player attrition + pod. `checkPlayerAttrition` in SystemView (called after every player hit, BEFORE the pod branch) mirrors the enemy logic: wingmen die lightest-hull-first as the pooled hull crosses thresholds computed fresh from the live roster via new `getShipHullContribution()` (exported from `fleetStats.js` — same math `totalHull` sums, so thresholds partition the pool exactly). On death: explosion at the lagged formation slot, splice from the `fleetShips` memo array in place (formation/firing/trails/render/presence all drop it that frame), pool max rebased down by the dead ship's share (so the later fetchShips ratio-preserve no-ops), then fire-and-forget the new **`POST /fitting/lose-ship`** endpoint (validates owner + NOT active ship + not stored + not pod; deletes the row) → `fetchShips()` reconciles; on failure the ship reappears (server is truth). Per-ship `loseShipInFlightRef` Set guards double-fire. Flagship death stays the existing `enter-pod` branch (its threshold is 0 — attrition runs first, so one huge overshoot hit kills escorts then pods). NO wreck/salvage from lost wingmen yet — deferred to F4 with the rest of stakes (wrecks 42P01). No migration.
  - **F4 SHIPPED (build-verified) 2026-07-06** — combat polish: server-validated loot + flagship payout + fleet-sticky targeting + battlefield tint. **(a) Server-validated loot (the A3 manifest):** the server now reproduces the client's deterministic pirate spawn per system — `src/game/galaxyGenerator.js` (verbatim copy of the client generator) + `src/game/pirateManifest.js` (hand-mirrored RNG stream of `generatePiratesForSystem` + the Sol spawn zones + loot tables) — yielding enemyId→credits manifests. New `/api/combat/enter-system` (resets the per-user per-system in-memory claim set — client fires it from the pirate-spawn effect, i.e. exactly when kills respawn) + `/api/combat/claim-loot` (validates enemy id ∈ manifest + unclaimed this visit, pays the SERVER's number; 404/409 rejects drop the wreck client-side). **The old trust-the-client `/fitting/award-loot` is REMOVED** — the credit-mint exploit is dead. Client wreck credits are display-only; wrecks carry `enemyId`+`systemId` (stamped on enemies at spawn to dodge closure staleness). Known gap: nothing verifies the fight *happened* (needs server combat); this caps loot to the real spawn per visit. **(b) Flagship payout:** flagship wreck pays its own loot + 50% of escorts' combined loot (`FLAGSHIP_FLEET_BONUS_FRAC`, same constant client `fleetEntities.js` + server manifest). **(c) Fleet-sticky targeting:** when the designated ship dies, lock auto-shifts to that fleet's next-to-die member (clears only when the fleet's gone); weapons prefer designated member → nearest living member of designated FLEET in range → nearest-any. **(d) Battlefield tint:** enemy glow color = fleet's current front defense layer (blue shield → bring kinetic, amber armor → bring laser, engine-color = hull exposed). No migration. ⚠ New CLAUDE.md pitfall #16: the two server game files are sync-copies of client code — mirror any client spawn/generator edit. Playtest notes: enemy damage tuning matters (losing fights cost real ships); old cached clients get 404 on loot until refresh after this deploys.

- **SHIPPED (build-verified) 2026-06-11 — Zoning generator: EVE-style REGIONS (spec step 4, no migration).** Galaxy partitioned into 10–15 **named regions** of organic shape (Voronoi cells around farthest-point seed systems), each tiered 1–5 by centroid-distance RANK (quantiles — raw distance collapsed into a mid-tier blob) + ±1 jitter for EVE-like pockets. Sol's region = "Core Worlds", always T1. Verified distribution at seed 12345: 15 regions, systems per tier 54/43/44/41/18, exotic stars avg region tier 3.9. **Danger now derives from region tier** (T_N → danger N-1..N; the old radial formula is gone); **star types re-roll tier-biased** (neutron/black-hole cluster deep); server deposits scale with danger — rarity bucket 2%→35%, quantity ×1→×2.25, quality bonus amplified +3→+6/level. **Fixed latent server bug:** `ensureDepositsExist`/`respawnDepletedDeposits` selected from celestial_bodies only, never JOINing star_systems — danger_level was silently 0 for every deposit ever spawned through that path. `ensure-body` now also refreshes a stored system's stale danger when the client reports a different value (handles the re-roll). UI: region name labels on the galaxy map at centroids (tier-colored, fog-gated to discovered regions), Region rows in map info panel + flight hover panel, enemy fleets stamped + named with region tier ("T3 Pirate Destroyer (Heavy Cannon)") per the relative-difficulty rule. **GALAXY RE-ROLLED** (user approved): procedural systems change character; fog-of-war + Sol unaffected; server DB rows for previously-visited systems get danger refreshed on next dock.

- **SHIPPED (build-verified) 2026-06-11 — Unified 5-tier module system + exotic recipe pass (spec step 3, migration 062).** Locked design: ONE 1–5 tier scale for modules / resource rarity / zones / enemy fleets (relative difficulty: +1 tier = crafty fight, +2 = high difficulty, +3 = deadly). Adds 18 modules incl. the **new armor family** (fits the shield slot — every defense slot is a shield-vs-armor choice), one weapon per damage type at T3 + T5, T3–T5 drives/reactors/shields + Resonance Mining Laser; 18 recipes keyed to rarity (T4/T5 require the previously-unused exotics — Dark Matter / Ancient Alloy / Quantum Dust / Void Essence — plus Plasma / Helium-3 / Solar Crystals); 7 tech nodes extend the research trees to **tier 4** (2400 / 6000 RP); `tech_shield_theory` + `tech_armor_eng` placeholders now actually unlock their modules. **T3+ is craft-only** (buy_price NULL) — the hard gate + the player-market supply-chain driver. **`combat_tuned` stats convention**: fit-module now snapshots `module_types.stats` into the fitted slot; client weapons.js + fleetStats.js prefer per-module damage/fire_rate/range/shield_hp/armor_hp ONLY when stats carry `"combat_tuned": true` — legacy modules keep type-default behavior exactly (zero balance drift), new tiers get real numbers. Tier badges: `utils/tiers.js` rarity color ramp + roman numerals, shown in the shared item tooltip. Scanner tiers renumbered onto the 1–5 scale (Wide-Field 2.5→3, Elite 3→4, Telemetry→4). **Run `npm run db:migrate` after deploy.** Note: until the zoning generator lands, exotics still spawn at the flat ~2% — T4/T5 crafts are intentionally near-impossible to farm today.
- Then per spec §7: **travel gating (warp-range)** → deep-enemy roster → stakes (ejection into wrecks — the loot-validation half of A3 shipped with F4; the cargo/module-ejection half still needs the wrecks `42P01` fix).
...
### Owner-requested 2026-09-17 (unranked, not yet specced)

- **Healing hulls.** Today hull has no regen and no in-system repair: it snaps back to max only on system change (`SystemView.jsx` ~2438) and on pod entry (~3522). Shield regens; armor + hull don't. The dead `/repair-cost` endpoint in `fitting.js` is the stub of an earlier attempt. Options to settle when specced: (a) paid repair at any station/dock (credits scale with damage × hull tier — the natural money sink), (b) slow passive hull regen out of combat (undercuts stakes; probably not), (c) a repair-nanite module / consumable for field repair, (d) a Repair skill in the catalog (`skill_definitions` — check for an existing `hull_repair_*` contract per pitfall #15). Should also decide whether wingmen heal the same way (pooled fleet hull → the pool max is rebased on wingman death, so "heal" must rebuild per-ship, not just top up the pool).
- **Weather on planets + effects.** Nothing exists — the generator only has `hasAtmosphere` (`galaxyGenerator.js` ~625). Needs a per-planet weather state (deterministic from seed + slow time cycle so all players see the same weather) and gameplay hooks: harvester yield modifiers, ground-scan precision penalty in storms, maybe landing/dock blocked during extreme weather, visual tint on the planet body + Scan tab readout. Server must own the modifier (harvester collect + ground survey run server-side) — mirror the generator per pitfall #16 if the client also computes it.
- **Star animation.** The system's star is a static circle + glow. Wants: corona flicker / pulsing glow, solar flares on exotic types (neutron pulse, black-hole accretion swirl per the tier-biased star types). Pure client render; keep it CSS/SMIL or a cheap per-frame transform — pitfall: per-frame allocations in SystemView (see `project_perf_delta_clamp`). Pairs with the existing CSS-twinkle starfield approach from the perf pass.
- **Planet graphics + spinning planets.** Planets are flat colored circles with a radial highlight (`SystemView.jsx` ~648–669, `#planetHighlight` gradient ~4216). Wants: per-type surface texture (bands for gas giants, ice caps, city lights on inhabited), a rotation effect (scrolling texture / pattern under a circular clip, or a moving terminator shadow), rings for some giants, atmosphere haze when `hasAtmosphere`. Same perf caution as star animation; also SystemMapWindow's dots (~1066) should stay simple.
...
- **Galaxy map v2** (queued 2026-06-12, after the perf/regions pass): multi-hop route planning (BFS over gate connections from current system to clicked system, highlight path + hop count in info panel — near-mandatory once warp-range gating lands); "center on me" button; click-empty-space-to-deselect; system name search box (pan-to-result); warp-range ring (ships with the travel-gating step); optional faint tier-colored region fills (judge after living with the border lines).
...
- **Possible crash during player-to-player trade (reported 2026-09-17, unreproduced).** Owner says "there might be a bug in trading that crashes the game" — no error text or steps yet. **Need from the reporter:** which step (invite / accept / adding items / confirm / after completion), whether the whole page went white (React render crash) or just the window, and the browser console error (F12 → Console, red text). Candidate spots to check once there's a repro: `TradeWindow.jsx` offer rendering (~97 `participant.offer` rows — a stack the other side sold/jettisoned mid-trade would leave a dangling `stack_id`), the server `trade:*` socket payloads after a reconnect (`utils/trade.js` `recoverActive()`), and the atomic-swap error path in `lib/trade.js` (a rejected swap should surface as "Trade Cancelled (reason)", not throw client-side).
...
### 2026-09-21 — Stored ship as flagship (found via "telemetry array does nothing"; no migration)

Owner's Starter Scout was **stored at a station AND `users.active_ship_id`** — the Ship Builder showed the Asteroid Telemetry Array fitted to "the active ship", the HUD named it, physics used its stats, but `fleetShips` (stored ships excluded, pitfall #15) never contained it, so nothing fitted to it counted and the telemetry tier read 0. Diagnosed with the console-exposed store (`__STORE__.getState().ships…`): `["Starter Scout","STORED","ACTIVE","HAS-TELEMETRY"]`. Cause: `/set-active-ship` never checked `storage_body_id` (the Ship Builder's Launch button / Fleet window could flag a parked hull), while `/store-ship` did refuse the active ship — one-way guard. Fixes: **(1)** `/set-active-ship` now refuses stored ships with "activate it from the station's Ships tab first"; **(2)** `/fitting/fleet` **self-heals**: if the flagship is stored, un-store it when the fleet has room, else move the flag to the first flying ship (logged); **(3)** client `useActiveShip` falls back to the first flying ship if the active one is stored; **(4)** `db:verify` counts users with a stored flagship. Debug lesson kept: the client store is on `window.__STORE__` — a two-command console check beat three rounds of guessing.
...
**Engines added 0 speed.** `recalcShipStats` read `thrust ?? speed` from engine module stats, but every seeded engine row only carries `speed_bonus` (engine_basic 0 → engine_void_5 110), so `computed_max_speed` was always the bare hull speed. Fixed to `thrust ?? speed ?? speed_bonus` in `api/fitting.js`, mirrored in `game/enemyManifest.js` (enemies use the same rule — pirates with Void Drives now genuinely outrun a bare hull, and so can players). Existing ships only recalc on fit/unfit, so new **`npm run db:recalc-ships`** (`src/db/recalc-ships.js`, exports + reuses `recalcShipStats`) refreshes every ship once; run it after this deploy. Expect a visible speed jump for anyone flying T2+ engines.

**Skill queue wiped partial progress (owner report).** Two causes in `POST /skills/queue/remove`: (1) removing ANY entry rescheduled every remaining entry from NOW — including the head — so its `started_at` reset and the in-progress level restarted; (2) removing the head itself discarded the live SP by design. Fix: entries before the removed slot keep their exact rows; only later entries re-chain. Removing the head **banks its live SP into `player_skills.sp`** (which may now sit between level snapshots); re-queueing that skill resumes from the bank via a *virtual earlier `started_at`* (`bankedOffsetMs` / `scheduleEntry`), so every existing formula (server live SP, client progress bars off started_at/finishes_at, the finishes_at pop check) shows the banked progress with no client change. Cascade: removing an entry also removes later entries of the same skill at higher levels (they'd otherwise train L3 with L2 never done). GET `/skills` now reports the head skill's LIVE sp so the per-skill bar in the Skills window advances. Response adds `removed_count` + `banked_sp`.
...
**Also new: `npm run db:rename -- OLDNAME NEWNAME`** (`src/db/rename-user.js`) — admin player rename from the DO console (no in-game feature). Updates `users.username`/`display_name` plus the denormalized `chat_messages.sender_name` + `activity_events.sender_name` in one transaction; same validation as registration. Multi-line pastes don't survive the DO web console, hence a script. **`npm run db:users [-- N]`** (`src/db/list-users.js`) lists every account (username, email, provider, credits, joined, last active) most-recently-active first. Note: `users.last_seen_at` is never written (`updateUserOnline` has no callers), so "last active" is derived from activity_events / chat_messages / player_system_visits.
