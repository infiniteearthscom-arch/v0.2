...
| **Zoning structure** | ~~Radial bands~~ → **EVE-style REGIONS** *(revised 2026-06-11)*: 10–15 named regions of organic, non-uniform shape (Voronoi cells around farthest-point seeds), each carrying a tier on the unified 1–5 scale. Tiers follow a core→rim gradient by distance RANK with jitter, so tier pockets form. Sol's region ("Core Worlds") is always T1. Chokepoint gating still applies via travel model. SHIPPED 2026-06-11. |
...
| ✅ 4 | **Zoning generator (EVE-style regions)** — 10–15 named Voronoi regions tiered 1–5 by distance rank + jitter; danger derives from region tier; tier-biased star placement; danger-scaled rarity (2%→35%) / quantity (×1→×2.25) / quality (+6/danger); region labels + info rows on map/flight views; enemy fleets stamped with region tier ("T3 Pirate Destroyer") | none | SHIPPED (build-verified) 2026-06-11. Galaxy re-rolled (user approved). Fixed latent bug: deposit spawns never read danger_level (missing star_systems JOIN). ensure-body now refreshes stale danger. |
