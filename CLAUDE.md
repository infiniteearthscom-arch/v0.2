...
8. **Migrations: no `CREATE EXTENSION`** — DO dev DB blocks it. Use `gen_random_uuid()` (built into PG 18), not `uuid_generate_v4()`. **Migration 009 was skipped.** Highest authored is `077_elite_loot_tables.sql` — next new migration is `078`. **Every deploy that includes a migration needs `npm run db:migrate` from the DO console, then `npm run db:verify`** — 064/065 sat unapplied for two months and 068 for two weeks because that step was skipped.
