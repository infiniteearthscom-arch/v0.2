...
8. **Migrations: no `CREATE EXTENSION`** — DO dev DB blocks it. Use `gen_random_uuid()` (built into PG 18), not `uuid_generate_v4()`. **Migration 009 was skipped.** Highest authored is `084_contracts_on_mission_board.sql` — next new migration is `085`. **Every deploy that includes a migration needs `npm run db:migrate` from the DO console, then `npm run db:verify`** — 064/065 sat unapplied for two months and 068 for two weeks because that step was skipped.
...
16. **The server's `src/game/galaxyGenerator.js` is a VERBATIM SYNC-COPY of `star-shipper/src/utils/galaxyGenerator.js`.** The server needs the same deterministic galaxy (system seeds / danger / region tier / bodies) to build enemy spawn manifests. Any edit to the client generator must be copied to the server in the same pass or spawns and loot drift. **Enemies themselves are NOT client code any more (Phase 2, 2026-09-17):** `src/game/enemyManifest.js` assembles them server-side from `enemy_templates` + `enemy_template_modules` (migration 069) using real `hull_types` / `module_types` rows, and `POST /api/combat/enter-system` returns the manifest the client hydrates (`utils/enemyManifest.js`). There is no client-side pirate generation and no RNG stream to mirror — to change enemies, edit the template rows. The stat math in `enemyManifest.js` deliberately mirrors the PLAYER's rules (`recalcShipStats` for shield/engine, client `weapons.js` for weapons, `fleetStats` MODULE_BONUSES for untuned modules); if you change how a player stat is computed, change the enemy side in the same commit.
