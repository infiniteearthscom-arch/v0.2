...
**Current state:** The full core gameplay loop is functional — mine → craft → fit → fly → trade → fight → explore. The player starts in Sol, can travel to any of 200 procedurally generated systems via an interstellar flight view or galaxy map, dock at planets/stations, scan for resources weighted by star type, and engage Void Reaver pirates with scaling danger levels. Two navigation modes exist: in-system flight (WASD around planets) and interstellar flight (WASD between star systems), with a galaxy map overlay for strategic route planning.
...
Migrations live in `star-shipper-server/migrations/` numbered sequentially (`001_init.sql` through `016_procedural_systems.sql`). Each migration is idempotent-ish — uses `CREATE TABLE IF NOT EXISTS`, `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`, etc. The user runs them manually against their PostgreSQL 18 instance.

**When creating migrations:** Always check existing schema first by reading the most recent migration files. The user's DB may have drifted from the migrations if they applied manual fixes.
...
### Tech Stack
- **Client:** React 18 + Vite, Zustand (state with immer + persist middleware), Tailwind CSS, no router
- **Server:** Node.js + Express, PostgreSQL 18, express-session for auth
...
star-shipper/                    # Vite React client
  src/
    ...
    utils/
      api.js                     # All API calls (~290 lines)
...
star-shipper-server/             # Express API server
  index.js                       # Server setup, session config, route mounting
...
### Migrations (001-016)
```
001_init.sql              — Users, auth tables
...
The `STAR_RESOURCE_MULTIPLIERS` table in `server/game/deposits.js` scales resource spawn weights by category based on star type. This means neutron star systems are rich in exotic materials but poor in biological resources, creating exploration incentives.
...
This bridges the client-side procedural generation with the server-side database.
...
4. **Migration ordering** — Always check the highest existing migration number (currently 016) before creating new ones. The user runs these manually and may have skipped some.
...
12. **Save/load UI** — DB persists but no explicit save slots or indication
